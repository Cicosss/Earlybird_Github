...
tables_to_check = ["continents", "countries", "leagues", "news_sources", "social_sources"]
...
def verify_foreign_key(
    parent_table: str, child_table: str, fk_column: str
...
for parent, child, fk in relationships:
    ...
    if total > 0:
        ...
        if invalid:
            print(
                f"   ⚠️  {len(invalid)} invalid records: {invalid[:3]}{'...' if len(invalid) > 3 else ''}"
            )
            all_valid = False
    else:
...
if os.path.exists(mirror_path):
    ...
    for table in ["continents", "countries", "leagues"]:
        ...
        if supabase_count != mirror_count:
            print(
                f"   ⚠️  {table:20s} : Supabase={supabase_count}, Mirror={mirror_count} (MISMATCH)"
            )
        else:
    ...
    if mirror_sources == supabase_news and mirror_news == 0:
        print(
            f"   ⚠️  news_sources      : Supabase={supabase_news}, Mirror(sources)={mirror_sources}, Mirror(news_sources)={mirror_news} (WRONG KEY)"
        )
    ...
    if supabase_social != mirror_social:
        print(
            f"   ⚠️  social_sources    : Supabase={supabase_social}, Mirror={mirror_social} (MISMATCH)"
        )
    else:
        print(
            f"   ✅ social_sources    : Supabase={supabase_social}, Mirror={mirror_social} (MATCH)"
        )
...
if os.path.exists(provider_file):
    ...
    print(
        f"   Found {news_sources_count} instances of '\"news_sources\": self.fetch_all_news_sources()'"
    )
...
if os.path.exists(mirror_path):
    ...
    print("   ❌ Mirror has WRONG key: 'sources' instead of 'news_sources'")
    print("   ❌ Mirror is MISSING 'social_sources' data")
else:
...
print()
print(
    f"   Plan accuracy: {correct_count}/{len(plan_claims)} claims verified ({correct_count / len(plan_claims) * 100:.1f}%)"
)
