...
class DeepFKVerifier:
    ...
    def check_orphaned_records(
        ...
        for record in child_records:
            ...
            if fk_value not in parent_ids:
                orphaned.append({"id": record.get("id"), f"{fk_column}": fk_value})
    ...
    def verify_countries_to_continents(self) -> dict[str, Any]:
        ...
        if orphaned_count > 0:
            self.issues.append(
                f"countries: {orphaned_count} orphaned records (invalid continent_id)"
            )
            if orphaned:
    ...
    def verify_leagues_to_countries(self) -> dict[str, Any]:
        ...
        # Check for orphaned records
        orphaned_count, orphaned = self.check_orphaned_records("leagues", "countries", "country_id")
        print(f"Orphaned records: {orphaned_count}")
        if orphaned_count > 0:
    ...
    def verify_news_sources_to_leagues(self) -> dict[str, Any]:
        ...
        if orphaned_count > 0:
            self.issues.append(
                f"news_sources: {orphaned_count} orphaned records (invalid league_id)"
            )
            if orphaned:
                self.issues.append(f"  Sample orphans: {orphaned[:5]}")
