...
def verify_settler_integration():
    ...
    try:
        ...
        if clv is not None:
            print_success("CLV calculation works correctly!")
            print_info(
                f"Test: odds_taken={test_odds_taken}, closing_odds={test_closing_odds}, CLV={clv:+.2f}%"
            )
            return True
        else:
...
def check_existing_data_quality():
    ...
    try:
        ...
        with get_db_context() as db:
            ...
            total_sent = db.query(NewsLog).filter(NewsLog.sent == True).count()

            with_alert_odds = db.query(NewsLog).filter(NewsLog.odds_at_alert.isnot(None)).count()

            with_kickoff_odds = (
                db.query(NewsLog).filter(NewsLog.odds_at_kickoff.isnot(None)).count()
            )

            print_info(f"Total sent alerts: {total_sent}")
            print_info(
                f"With odds_at_alert: {with_alert_odds} ({with_alert_odds / total_sent * 100:.1f}%)"
            )
            print_info(
                f"With odds_at_kickoff: {with_kickoff_odds} ({with_kickoff_odds / total_sent * 100:.1f}%)"
            )
