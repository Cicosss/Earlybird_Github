...
class SupabaseValidator:
    ...
    def verify_foreign_key(
        self, child_table: str, parent_table: str, fk_column: str
    ) -> tuple[int, int, bool]:
        """
        ...
        try:
            ...
            for record in child_records:
                ...
                if fk_value in parent_ids:
                    ...
                    invalid_count += 1
                    invalid_records.append({"id": record["id"], f"{fk_column}": fk_value})
            ...
            self.errors.append(
                f"Failed to verify FK {child_table}.{fk_column} -> {parent_table}: {e}"
            )
            return 0, 0, False
    ...
    def get_table_schema(self, table_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            if response.data:
                return {"columns": list(response.data[0].keys()), "sample_record": response.data[0]}
    ...
    def validate_continents(self) -> dict[str, Any]:
        ...
        if count != expected_count:
            self.errors.append(f"continents: Expected {expected_count} records, found {count}")
    ...
    def validate_countries(self) -> dict[str, Any]:
        ...
        expected_count = 28
        if count != expected_count:
            self.errors.append(f"countries: Expected {expected_count} records, found {count}")
    ...
    def validate_leagues(self) -> dict[str, Any]:
        ...
        expected_count = 56
        if count != expected_count:
            self.errors.append(f"leagues: Expected {expected_count} records, found {count}")
    ...
    def validate_news_sources(self) -> dict[str, Any]:
        ...
        expected_count = 140
        if count != expected_count:
            self.errors.append(f"news_sources: Expected {expected_count} records, found {count}")
        ...
        try:
            ...
            for table_name in ["continents", "countries", "leagues", "news_sources"]:
                ...
                if success:
                    ...
                    print(
                        f"{status} {table_name}: Mirror={mirror_count}, Supabase={supabase_count}"
                    )
    ...
    with open(output_path, "w") as f:
        ...
        for table_name, table_result in results["results"].items():
            ...
            if "count" in table_result:
                f.write(
                    f"- Records: {table_result['count']} (expected: {table_result.get('expected_count', 'N/A')})\n"
                )
