...
class TestV71SerperConstants:
    ...
    def test_serper_rate_limit_delay_slow_constant_exists(self):
        ...
        from src.processing.news_hunter import SERPER_RATE_LIMIT_DELAY, SERPER_RATE_LIMIT_DELAY_SLOW
    ...
    def test_constants_used_in_requests(self):
        ...
        assert timeout_uses >= 5, (
            f"Expected >= 5 uses of SERPER_REQUEST_TIMEOUT, found {timeout_uses}"
        )
        assert delay_uses >= 4, f"Expected >= 4 uses of SERPER_RATE_LIMIT_DELAY, found {delay_uses}"
        assert delay_slow_uses >= 1, (
            f"Expected >= 1 use of SERPER_RATE_LIMIT_DELAY_SLOW, found {delay_slow_uses}"
        )
...
class TestV71SearchTypeSummary:
    ...
    def test_tier1_search_types_list_exists(self):
        ...
        assert "tier1_search_types = [" in content, "tier1_search_types should be defined as a list"
    ...
    def test_no_obsolete_twitter_hack(self):
        ...
        tier1_items = [
            line.strip() for line in tier1_def.split("\n") if not line.strip().startswith("#")
        ]
    ...
    def test_twitter_intel_cache_in_tier1(self):
        ...
        tier1_def = match.group(1)
        # Check for double quotes (Python dict format)
    ...
    def test_dynamic_country_in_tier1(self):
        ...
        tier1_def = match.group(1)
        # Check for double quotes (Python dict format)
        assert '"dynamic_country"' in tier1_def, "'dynamic_country' should be in tier1_search_types"
    ...
    def test_all_actual_search_types_accounted(self):
        ...
        # Find all search_type assignments (handle both single and double quotes around key)
        search_types_used = set(
            re.findall(r'["\']search_type["\']:\s*["\']([^"\']+)["\']', content)
        )
