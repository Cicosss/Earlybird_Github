...
def test_sporting_integration_flow():
    ...
    if ai_extracted_name in provider.MANUAL_MAPPING:
        ...
        print("   ❌ Not found in MANUAL_MAPPING")
        return False
    ...
    if team_details:
        ...
        if "nextMatch" in team_details:
            ...
            if next_match:
                print("      Next Match: Available")
            else:
                print("      Next Match: None (team might not have upcoming matches)")
    else:
        print("   ⚠️  Warning: Could not retrieve team details")
        print("      This might be due to API rate limiting or team not having data")
        # Don't fail, just warn
    ...
    for variant, description in test_variants:
        ...
        if team_id:
            ...
            if fotmob_name == "Sporting CP":
                print("   ✅ Correct canonical name: 'Sporting CP'")
            else:
                print(f"   ⚠️  Warning: Resolved to '{fotmob_name}' (expected 'Sporting CP')")
        else:
    ...
    if conflicts:
        ...
        print("   Note: This is expected for team name variants")
    else:
