...
class TestContinentalOrchestratorResilience:
    ...
    def test_mirror_data_integrity(self):
        ...
        for country in mirror_data["countries"]:
            ...
            assert continent_id in continents, (
                f"Country {country['name']} references invalid continent {continent_id}"
            )

        # Check that all leagues reference valid countries
        for league in mirror_data["leagues"]:
            country_id = league["country_id"]
            assert country_id in countries, (
                f"League {league['api_key']} references invalid country {country_id}"
            )
...
class TestContinentalOrchestratorEndToEnd:
    ...
    def test_source_tracking(self, log_capture):
        ...
        if result["source"] == "supabase":
            ...
            assert has_supabase_log, (
                f"Expected Supabase-related log not found. Source: {result['source']}. Logs: {log_capture.format_all()}"
            )
            ...
            has_mirror_log = log_capture.contains(
                "Loaded mirror", level="INFO"
            ) or log_capture.contains("fallback", level="WARNING")
            has_supabase_log = log_capture.contains("Supabase", level="INFO")
