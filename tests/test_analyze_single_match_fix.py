...
def test_5_create_test_match():
    ...
    try:
        ...
        try:
            ...
            existing_match = db.query(Match).filter(Match.id == "test_radar_match_001").first()

            if existing_match:
                logger.info(
                    f"✅ Test match already exists: {existing_match.home_team} vs {existing_match.away_team}"
                )
                return True
            ...
            logger.info(
                f"✅ PASS: Test match created: {test_match.home_team} vs {test_match.away_team}"
            )
            return True
        finally:
            db.close()
...
"""
        ...
        result = main_module.analyze_single_match(
            match_id="test_radar_match_001", forced_narrative=forced_narrative
        )
        ...
        logger.info("✅ PASS: Function called successfully")
...
def test_7_call_with_invalid_match():
    ...
    try:
        ...
        result = main_module.analyze_single_match(
            match_id="invalid_match_id_12345", forced_narrative=None
        )
        ...
        if "not found" not in result["error"].lower():
            logger.error(
                f"❌ FAIL: Error message doesn't indicate match not found: {result['error']}"
            )
            return False

        logger.info("✅ PASS: Invalid match_id handled correctly")
        logger.info(f"   Error: {result['error']}")
...
def test_8_verify_newslog_created():
    ...
    try:
        ...
        try:
            ...
            radar_logs = (
                db.query(NewsLog)
                .filter(NewsLog.match_id == "test_radar_match_001", NewsLog.source == "radar")
                .all()
            )
