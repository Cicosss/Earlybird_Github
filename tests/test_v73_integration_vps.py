...
def test_optimizer_singleton_consistency_across_components():
    ...
    from src.analysis.optimizer import _weight_cache, get_optimizer
...
def test_tavily_singleton_consistency_across_components():
    ...
    if provider1._shared_cache:
        assert provider2._shared_cache is provider1._shared_cache, (
            "Shared cache deve essere condivisa"
        )
        assert provider3._shared_cache is provider1._shared_cache, (
            "Shared cache deve essere condivisa"
        )
...
def test_optimizer_cache_survives_multiple_cycles():
    ...
    from src.analysis.optimizer import WEIGHTS_FILE, _weight_cache, get_optimizer
...
def test_vps_environment_compatibility():
    ...
    try:
        from src.analysis.optimizer import OptimizerWeightCache, get_optimizer
        from src.ingestion.tavily_provider import TavilyProvider
        from src.utils.shared_cache import get_shared_cache
