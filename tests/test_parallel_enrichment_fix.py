...
def test_5_parallel_enrichment():
    ...
    try:
        ...
        if result.successful_calls == expected_total:
            logger.info(
                f"✅ PASS: Parallel enrichment completed with {result.successful_calls}/{expected_total} successful"
            )
            logger.info("All tasks completed successfully!")
        elif result.successful_calls == 9:
            logger.info(
                f"⚠️ PARTIAL: Parallel enrichment completed with {result.successful_calls}/{expected_total} successful"
            )
            logger.info(
                "9/10 successful suggests that the weather task was skipped (stadium_coords not available)"
            )
            ...
            logger.warning(
                f"⚠️ WARNING: Parallel enrichment completed with {result.successful_calls}/{expected_total} successful"
            )
