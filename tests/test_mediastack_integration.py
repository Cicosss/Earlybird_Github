...
class TestSearchProviderMediastackIntegration:
    ...
    def test_mediastack_used_as_last_fallback(self):
        ...
        with (
            patch("src.ingestion.search_provider._BRAVE_AVAILABLE", False),
...
class TestMediastackKeyRotation:
    ...
    def test_key_rotation_on_429_error(self):
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            mock_response_200.json.return_value = {
                "data": [{"title": "Result", "url": "http://test.com"}]
            }
    ...
    def test_key_rotation_monthly_reset(self):
        ...
        with patch(
            "src.ingestion.mediastack_key_rotator.MEDIASTACK_API_KEYS",
            ["key1", "key2", "key3", "key4"],
        ):
            from src.ingestion.mediastack_key_rotator import MediaStackKeyRotator
...
class TestMediastackCircuitBreaker:
    ...
    def test_circuit_breaker_blocks_requests_when_open(self):
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            provider._circuit_breaker._state.last_failure_time = (
                10000000000  # Future time to prevent recovery
            )
...
class TestMediastackCaching:
    ...
    def test_cache_hit_returns_cached_response(self):
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            mock_response.status_code = 200
            mock_response.json.return_value = {
                "data": [{"title": "Cached Result", "url": "http://test.com"}]
            }
    ...
    def test_cache_miss_fetches_from_api(self):
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            mock_response.status_code = 200
            mock_response.json.return_value = {
                "data": [{"title": "API Result", "url": "http://test.com"}]
            }
...
class TestMediastackDeduplication:
    ...
    def test_duplicate_query_returns_empty(self):
        ...
        with (
            patch("src.ingestion.mediastack_provider._SHARED_CACHE_AVAILABLE", True),
            patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True),
        ):
            provider = create_mock_mediastack_provider()
    ...
    def test_search_filters_wrong_sport_results(self):
        ...
        mock_response.json.return_value = {
            "data": [
                ...
                {
                    "title": "NBA: Lakers injury report",
                    "url": "https://nba.com/1",
                    "description": "Basketball news",
                },
                # Should be filtered OUT (women's football)
                {
                    "title": "WSL: Chelsea Women lineup",
                    "url": "https://wsl.com/1",
                    "description": "Women's football",
                },
                # Should PASS (men's football)
                {
                    "title": "Serie A: Milan injury update",
                    "url": "https://seria.com/1",
                    "description": "Football news",
                },
                # Should PASS (men's football)
                {
                    "title": "Premier League squad rotation",
                    "url": "https://pl.com/1",
                    "description": "EPL news",
                },
            ]
        }
...
class TestMediastackRegressionV45:
    ...
    def test_basketball_not_in_football_results(self):
            ...
            "data": [
                {
                    ...
                    "url": "https://nba.com/draft",
                    "description": "Basketball draft",
                },
                {
                    "title": "Euroleague Final Four",
                    "url": "https://euroleague.com/f4",
                    "description": "Basketball tournament",
                },
                {
                    "title": "Serie A: Juventus lineup",
                    "url": "https://juve.com/1",
                    "description": "Football news",
                },
            ]
        }
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            results = provider.search_news(
                "Serie A injury -basket -basketball -euroleague -nba", limit=5
            )
    ...
    def test_womens_football_not_in_mens_results(self):
        ...
        mock_response.json.return_value = {
            "data": [
                {
                    "title": "WSL: Arsenal Women injury",
                    "url": "https://wsl.com/1",
                    "description": "Women's football",
                },
                {
                    "title": "Liga F: Barcelona Femminile",
                    "url": "https://ligaf.com/1",
                    "description": "Calcio femminile",
                },
                {
                    "title": "Premier League: Arsenal injury",
                    "url": "https://pl.com/1",
                    "description": "Men's football",
                },
            ]
        }
        ...
        with patch("src.ingestion.mediastack_provider.MEDIASTACK_ENABLED", True):
            ...
            results = provider.search_news("Arsenal injury -women -femminile -wsl -liga f", limit=5)
