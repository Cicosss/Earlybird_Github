...
team_name_strategy = st.text(
    alphabet=st.characters(whitelist_categories=("L", "N", "Zs")), min_size=1, max_size=50
...
date_strategy = st.one_of(st.none(), st.dates().map(lambda d: d.strftime("%Y-%m-%d")))
...
class TestProperty1DisabledProviderReturnsNone:
    ...
    **Validates: Requirements 1.3**
    ...
    @given(home_team=team_name_strategy, away_team=team_name_strategy, match_date=date_strategy)
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(news_title=st.text(min_size=1, max_size=100), team_name=team_name_strategy)
    def test_verify_news_item_returns_none_when_disabled(
        ...
        **Validates: Requirements 1.3**
        ...
        result = disabled_provider.verify_news_item(
            news_title=news_title, news_snippet="Test snippet", team_name=team_name
        )
        assert result is None
    ...
    @settings(max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @given(home_team=team_name_strategy, away_team=team_name_strategy)
    def test_enrich_match_context_returns_none_when_disabled(
        ...
        **Validates: Requirements 1.3**
        ...
        result = disabled_provider.enrich_match_context(
            home_team=home_team, away_team=away_team, match_date="2026-01-15", league="Serie A"
        )
        assert result is None
    ...
    @given(
        handles=st.lists(
            st.text(min_size=2, max_size=20).map(lambda x: f"@{x}"), min_size=1, max_size=5
        )
    )
    def test_extract_twitter_intel_returns_none_when_disabled(self, handles, disabled_provider):
        """
        **Feature: deepseek-migration, Property 1: Disabled Provider Returns None**
        **Validates: Requirements 1.3**
        """
    ...
    **Validates: Requirements 5.2, 5.3**
    ...
    def test_brave_results_included_when_available(self, base_prompt, brave_results):
        ...
        **Validates: Requirements 5.2, 5.3**
        ...
        assert (
            "web search results" in final_prompt.lower()
            or "provided sources" in final_prompt.lower()
        )
    ...
    **Validates: Requirements 7.1**
    ...
    def test_deep_dive_invalid_home_team(self, invalid_team, mock_api_key):
        ...
        **Validates: Requirements 7.1**
        ...
        result = provider.get_match_deep_dive(home_team=invalid_team, away_team="Valid Team")
        assert result is None
    ...
    def test_deep_dive_invalid_away_team(self, invalid_team, mock_api_key):
        ...
        **Validates: Requirements 7.1**
        ...
        result = provider.get_match_deep_dive(home_team="Valid Team", away_team=invalid_team)
        assert result is None
    ...
    def test_verify_news_no_title_no_snippet(self, mock_api_key):
        ...
        **Validates: Requirements 7.1**
        ...
        result = provider.verify_news_item(news_title="", news_snippet="", team_name="Inter")
        assert result is None
    ...
    def test_verify_news_no_team_name(self, mock_api_key):
        ...
        **Validates: Requirements 7.1**
        ...
        result = provider.verify_news_item(
            news_title="Some news", news_snippet="Some snippet", team_name=""
        )
        assert result is None
