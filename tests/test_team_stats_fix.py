...
def test_get_team_details_by_name():
    ...
    with patch.object(provider, "search_team_id", return_value=(12345, "Hearts")):
        ...
        with patch.object(
            provider,
            "get_team_details",
            return_value={"team_id": 12345, "name": "Hearts", "squad": {}, "fixtures": {}},
            ...
            print("✅ get_team_details_by_name correctly converts team_name to team_id")
...
def test_get_team_stats():
    ...
    with patch.object(
        ...
        print("✅ get_team_stats returns proper structure")
        print(f"   Result: {result}")
        return True
...
def test_get_team_stats_with_error():
    ...
    with patch.object(
        provider,
        "get_team_details_by_name",
        return_value={"_error": True, "_error_msg": "Team not found"},
    ):
        ...
        print("✅ get_team_stats handles errors gracefully")
        print(f"   Result: {result}")
        return True
...
def test_get_full_team_context():
    ...
    with patch.object(
        ...
        with patch.object(
            provider,
            "get_table_context",
            return_value={"zone": "Mid-table", "position": 6, "motivation": "Medium"},
        ):
            ...
            print("✅ get_full_team_context uses the wrapper correctly")
            print(f"   Result: {result}")
            return True
...
def test_get_stadium_coordinates():
    ...
    with patch.object(
        ...
        return_value={
            "team_id": 12345,
            "name": "Hearts",
            "stadium": {"lat": 55.8642, "lon": -3.1879},
        },
    ):
        ...
        print("✅ get_stadium_coordinates uses the wrapper correctly")
        print(f"   Result: {result}")
        return True
...
def test_get_turnover_risk():
    ...
    with patch.object(
        provider,
        "get_team_details_by_name",
        return_value={"team_id": 12345, "name": "Hearts", "squad": {}},
    ):
        result = provider.get_turnover_risk("Hearts")

        # Result may be None (no turnover risk data available)
        print("✅ get_turnover_risk uses the wrapper correctly")
        print(f"   Result: {result}")
        return True
