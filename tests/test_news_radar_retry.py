...
async def test_retry_logic_on_timeout():
    ...
    print("     - Expected attempts: 3 (1 initial + 2 retries)")
...
async def test_retry_logic_on_network_error():
    ...
    print("     - Expected attempts: 2 (1 initial + 1 retry)")
    print(f"     - Result: {'SUCCESS' if result else 'FAILURE'}")
...
async def test_backward_compatibility():
    ...
    def mock_post_success(*args, **kwargs):
        ...
        print("  ↳ Attempt #1: Simulating success...")
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "choices": [
    ...
    print("\n  📊 Results:")
    print(f"     - Result: {'SUCCESS' if result else 'FAILURE'}")
    print("     - Uses default timeout=60 and max_retries=2")
