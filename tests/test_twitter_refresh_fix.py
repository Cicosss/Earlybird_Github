...
def test_imports():
    ...
    try:
        from src.main import (
            _DEEPSEEK_PROVIDER_AVAILABLE,
            _TWITTER_INTEL_AVAILABLE,
        )
...
def test_cache_instance():
    ...
    try:
        ...
        logger.info("✅ TwitterIntelCache instance created")
...
def test_method_signature():
    ...
    try:
        ...
        if hasattr(TwitterIntelCache, "refresh_twitter_intel"):
            method = TwitterIntelCache.refresh_twitter_intel
            is_async = inspect.iscoroutinefunction(method)
            logger.info("✅ refresh_twitter_intel() exists")
