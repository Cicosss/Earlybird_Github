...
player_name_st = st.text(
    alphabet=st.characters(whitelist_categories=("L", "N", "Zs")), min_size=2, max_size=30
...
team_name_st = st.text(
    alphabet=st.characters(whitelist_categories=("L", "N", "Zs")), min_size=2, max_size=50
).filter(lambda x: x.strip())
    ...
    *For any* team with avg_goals_scored < 1.0, is_low_scoring() returns True.
    ...
    if avg < LOW_SCORING_THRESHOLD:
        assert form.is_low_scoring() is True, (
            f"Team with avg {avg} goals should be low_scoring (threshold={LOW_SCORING_THRESHOLD})"
        )
    else:
...
def test_form_stats_losing_streak_detection(wins: int, remaining: int):
    ...
    *For any* team with 0 wins in last 5 matches, is_on_losing_streak() returns True.
    ...
    if wins == 0 and form.matches_played >= 5:
        assert form.is_on_losing_streak() is True, "Team with 0 wins should be on losing streak"
    elif wins > 0:
...
def test_verified_data_key_player_aggregation(
    ...
    *For any* set of player impacts, the total key player impact should
    ...
    """
    home_players = [PlayerImpact(name=name, impact_score=score) for name, score in home_impacts]
    away_players = [PlayerImpact(name=name, impact_score=score) for name, score in away_impacts]
    ...
    expected_home_key = sum(
        score for _, score in home_impacts if score >= PLAYER_KEY_IMPACT_THRESHOLD
    )
    expected_away_key = sum(
        score for _, score in away_impacts if score >= PLAYER_KEY_IMPACT_THRESHOLD
    )
...
class MockTavilyVerifier:
    ...
    def query(self, request) -> dict | None:
        self._call_count += 1
        if self._should_fail:
            return None
        return {"answer": "test", "results": [], "provider": "tavily"}

    def query_optimized(self, request) -> dict | None:
    ...
    def query_with_fallback(self, request) -> dict | None:
...
def test_property_13_provider_fallback(
    ...
    *For any* Tavily API failure, the system SHALL attempt Perplexity
    ...
    mock_tavily = MockTavilyVerifier(should_fail=tavily_fails, is_available=tavily_available)
    mock_perplexity = MockPerplexityVerifier(
        should_fail=perplexity_fails, is_available=perplexity_available
    )
...
def test_property_4_under_market_recommendation():
    ...
    *For any* verification where both teams have last5_goals_avg < 1.0 AND
    ...
    has_under = (result.recommended_market and "under" in result.recommended_market.lower()) or any(
        "under" in alt.lower() for alt in result.alternative_markets
    )
...
def test_property_2_market_reconsideration_flag():
    ...
    *For any* verification where total key_player impact > 20 AND suggested
    ...
    if total_key_impact > CRITICAL_IMPACT_THRESHOLD:
        ...
        has_reconsideration = (
            result.status == VerificationStatus.CHANGE_MARKET
            or result.status == VerificationStatus.REJECT
            or len(result.inconsistencies) > 0
        )
...
def test_run_verification_check_helper_available():
    ...
    """
    try:
        from src.main import _VERIFICATION_LAYER_AVAILABLE, run_verification_check
...
def test_bug6_referee_empty_string_regression():
    ...
    result_no_context = verifier._parse_referee_stats(
        text="Some random text without referee context", referee_name=""
    )
    ...
    result_with_pattern = verifier._parse_referee_stats(
        text="The referee averages 5.0 cards per game this season", referee_name=""
    )
    ...
    result_none = verifier._parse_referee_stats(
        text="Some random text without referee context", referee_name=None
    )
    ...
    result_valid = verifier._parse_referee_stats(
        text="Referee John Smith averages 5.2 cards per game this season", referee_name="John Smith"
    )
...
def test_bug3_form_regex_no_N_character_regression():
    ...
    result_valid = verifier._parse_form_stats(
        text="Team FC last 5 matches: W-W-D-L-L scored 8 goals", team_name="Team FC"
    )
    if result_valid:
...
def test_v21_parser_generic_team_names():
    ...
    assert result.home_form.draws == 2, "Flamengo draws should be 2"
    assert result.home_form.losses == 1, "Flamengo losses should be 1"
...
def test_v21_parser_italian_teams_regression():
    ...
    assert result.home_form.wins == 3, "Inter wins should be 3"
    assert result.away_form.wins == 2, "Milan wins should be 2"

    # Player impacts
    assert result.home_player_impacts[0].impact_score == 10, "Lautaro (€80m) should be impact 10"
    assert result.away_player_impacts[0].impact_score == 9, "Leao (€70m) should be impact 9"
...
def test_v22_parser_corner_extraction():
    ...
    parser = OptimizedResponseParser(
        home_team="Flamengo", away_team="Palmeiras", referee_name="Test Ref", players=[]
    )
    ...
    assert result.home_corner_avg == 5.0, (
        f"Flamengo corners should be 5, got {result.home_corner_avg}"
    )
    assert result.away_corner_avg == 8.0, (
        f"Palmeiras corners should be 8, got {result.away_corner_avg}"
    )
...
def test_v22_fuzzy_team_matching():
    ...
    """
    from src.utils.text_normalizer import find_team_in_text, fuzzy_match_team
...
def test_v222_form_written_numbers():
    ...
    parser = OptimizedResponseParser(
        home_team="Galatasaray", away_team="Fenerbahçe", referee_name=None, players=[]
    )
    ...
    assert result.home_form.draws == 2, (
        f"Galatasaray draws should be 2, got {result.home_form.draws}"
    )
...
def test_v25_form_has_won_drawn_pattern():
    ...
    assert result.home_form is not None, "Galatasaray form should be extracted"
    assert result.home_form.wins == 4, f"Galatasaray wins should be 4, got {result.home_form.wins}"
    assert result.home_form.draws == 1, (
        f"Galatasaray draws should be 1, got {result.home_form.draws}"
    )
    assert result.home_form.losses == 0, (
        f"Galatasaray losses should be 0, got {result.home_form.losses}"
    )
    ...
    assert result.away_form.draws == 2, (
        f"Fenerbahçe draws should be 2, got {result.away_form.draws}"
    )
...
def test_v25_fallback_threshold_75_inclusive():
    ...
    """
    from src.analysis.verification_layer import FormStats, TavilyVerifier, VerifiedData
...
def test_v26_execute_perplexity_fallback_returns_corner_data():
    ...
    with patch("src.analysis.verification_layer.PERPLEXITY_AVAILABLE", True):
        with patch(
            "src.analysis.verification_layer.get_perplexity_provider", return_value=mock_perplexity
        ):
    ...
    assert result["home_corners_avg"] == 5.2, (
        f"Expected home_corners_avg=5.2, got {result['home_corners_avg']}"
    )
    assert result["away_corners_avg"] == 4.8, (
        f"Expected away_corners_avg=4.8, got {result['away_corners_avg']}"
    )
...
def test_v26_query_with_fallback_calls_perplexity_when_corners_missing():
    ...
    mock_tavily_provider.search.return_value = MagicMock(
        answer="Test team won 3, drew 1, lost 1. No corner data available.", results=[]
    )
...
def test_v26_orchestrator_integrates_perplexity_corners():
    ...
    mock_tavily_provider = MagicMock()
    mock_tavily_provider.search.return_value = MagicMock(
        answer="Wisła Kraków won 2, drew 2, lost 1. Lech Poznań won 3, drew 1, lost 1.", results=[]
    )
    ...
    orchestrator = VerificationOrchestrator(
        tavily_verifier=tavily_verifier, use_optimized_queries=True
    )
...
def test_v26_perplexity_rescue_when_tavily_fails_completely():
    ...
    assert result.get("perplexity_fallback_executed") == True, (
        "Perplexity should have been executed"
    )
    assert result.get("provider") == "perplexity_v2.6_rescue", (
        f"Expected provider 'perplexity_v2.6_rescue', got {result.get('provider')}"
    )
