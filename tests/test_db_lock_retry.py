...
from src.database.models import get_db_session, with_db_retry
...
class TestMigrationDbLock:
    ...
    def test_migration_uses_timeout(self):
        ...
        with open("src/database/migration.py") as f:
            source = f.read()
    ...
    def test_migration_check_and_migrate_signature(self):
        ...
        for param in sig.parameters.values():
            if param.default is inspect.Parameter.empty:
                pytest.fail(
                    f"Parametro {param.name} non ha default - potrebbe rompere chiamate esistenti"
                )
