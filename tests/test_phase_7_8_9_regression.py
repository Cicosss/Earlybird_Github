...
class TestPhase7Optimizer:
    ...
    def test_optimizer_state_machine(self):
        ...
        from src.analysis.optimizer import OptimizerState, get_optimizer_state
    ...
    def test_weight_bounds(self):
        ...
        from src.analysis.optimizer import MAX_WEIGHT, MIN_WEIGHT, calculate_advanced_weight

        # Extreme positive ROI
        weight = calculate_advanced_weight(
            roi=1.0, sharpe=2.0, max_drawdown=0.0, n_samples=60, sortino=3.0
        )
        assert MIN_WEIGHT <= weight <= MAX_WEIGHT

        # Extreme negative ROI
        weight = calculate_advanced_weight(
            roi=-1.0, sharpe=-1.0, max_drawdown=-0.5, n_samples=60, sortino=-1.0
        )
        assert MIN_WEIGHT <= weight <= MAX_WEIGHT

    def test_frozen_state_returns_neutral_weight(self):
        """FROZEN state should always return 1.0 weight."""
        from src.analysis.optimizer import NEUTRAL_WEIGHT, calculate_advanced_weight

        # Even with great ROI, FROZEN state should return 1.0
        weight = calculate_advanced_weight(
            roi=0.5, sharpe=2.0, max_drawdown=0.0, n_samples=10, sortino=3.0
        )
...
class TestPhase8AlertDispatch:
    ...
    def test_score_below_threshold_never_alerts(self):
        ...
        should_alert = score >= ALERT_THRESHOLD_HIGH and (
...
class TestPhase9Settlement:
    ...
    def test_evaluate_bet_home_win(self):
        ...
        from src.analysis.settler import RESULT_LOSS, RESULT_WIN, evaluate_bet
    ...
    def test_evaluate_bet_cancelled_match(self):
        ...
        from src.analysis.settler import RESULT_PUSH, evaluate_bet
...
class TestPushOutcomeHandling:
    ...
    def test_push_outcome_does_not_count_as_loss(self):
        ...
        try:
            ...
            assert profit_after_push == 1.0, (
                f"PUSH should not change profit (got {profit_after_push})"
            )
...
class TestOptimizerV52Fixes:
    ...
    def test_record_bet_result_none_league_or_market(self):
        ...
        try:
            ...
            optimizer.record_bet_result(league=None, market="Over 2.5", outcome="WIN", odds=1.9)

            optimizer.record_bet_result(league="test", market=None, outcome="WIN", odds=1.9)

            optimizer.record_bet_result(league="", market="Over 2.5", outcome="WIN", odds=1.9)
...
class TestSettlerV53Fixes:
    ...
    def test_tavily_response_with_none_title_content(self):
        ...
        result = _tavily_post_match_search(
            home_team="Test Home", away_team="Test Away", match_date=datetime.now(timezone.utc)
        )
    ...
    def test_evaluate_over_under_edge_cases(self):
        ...
        """
        from src.analysis.settler import RESULT_PENDING, evaluate_over_under
...
class TestNoneParameterHandling:
    ...
    def test_evaluate_bet_with_none_scores(self):
        ...
        """
        from src.analysis.settler import RESULT_PENDING, evaluate_bet
    ...
    def test_evaluate_bet_with_string_scores(self):
        ...
        """
        from src.analysis.settler import RESULT_WIN, evaluate_bet
...
class TestOddsTypeConversion:
    ...
    def test_record_bet_result_with_string_odds(self):
        ...
        try:
            ...
            assert abs(stats.get("profit", 0) - 1.10) < 0.01, (
                f"Profit should be 1.10, got {stats.get('profit', 0)}"
            )
    ...
    def test_record_bet_result_with_invalid_odds_type(self):
        ...
        try:
            ...
            assert abs(stats.get("profit", 0) - 0.9) < 0.01, (
                f"Should use default odds 1.9, got profit {stats.get('profit', 0)}"
            )
...
class TestEndToEndIntegration:
    ...
    def test_full_settlement_to_optimizer_flow(self):
        ...
        try:
            ...
            assert abs(serie_a_stats.get("profit", 0) - 0.85) < 0.01, (
                "Profit should be ~0.85 (1.85 - 1)"
            )

            # Verify Premier League 1X2 with string odds was converted correctly
            pl_stats = (
                optimizer.data["stats"].get("soccer_england_premier_league", {}).get("1X2", {})
            )
            ...
            turkey_stats = (
                optimizer.data["stats"].get("soccer_turkey_super_league", {}).get("OVER", {})
            )
            assert turkey_stats.get("bets", 0) == 1, (
                "Turkey OVER should have 1 bet (Over 0.5 categorized correctly)"
            )
            ...
            assert "INJURY_INTEL" in optimizer.data["drivers"], (
                "INJURY_INTEL driver should be tracked"
            )
    ...
    def test_apply_weight_integration_with_real_data(self):
        ...
        try:
            ...
            score, msg = optimizer.apply_weight_to_score(
                base_score=8.5, league=None, market="Over 2.5"
            )
            ...
            score, msg = optimizer.apply_weight_to_score(
                base_score=8.5, league="soccer_unknown_league", market="Over 2.5"
            )
