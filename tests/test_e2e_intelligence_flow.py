...
class TestSportFiltering:
    ...
    def test_search_provider_exclusion_terms(self):
        ...
        for term in critical_exclusions:
            assert term in SPORT_EXCLUSION_TERMS, f"Termine esclusione mancante: {term}"
...
class TestEndToEndFlow:
    ...
    def test_news_to_analyzer_flow(self):
        ...
        news_snippet = "\n".join(
            [f"[{n['source']}] {n['title']}: {n['snippet']}" for n in news_items]
        )
    ...
    def test_tier_priority_order(self):
        ...
        if browser_pos > 0 and search_pos > 0:
            assert browser_pos < search_pos, (
                "Browser Monitor (TIER 0) deve essere prima di Search (TIER 1)"
            )
