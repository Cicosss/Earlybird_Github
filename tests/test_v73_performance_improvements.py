...
def test_optimizer_cache_invalidation_after_settlement():
    ...
    from src.analysis.optimizer import WEIGHTS_FILE, StrategyOptimizer, _weight_cache
...
def test_tavily_shared_cache_deduplication():
    ...
    with patch("src.ingestion.tavily_provider.TAVILY_ENABLED", True):
        with patch(
            "src.ingestion.tavily_provider.get_shared_cache", return_value=mock_shared_cache
        ):
            with patch(
                "src.ingestion.tavily_provider.get_http_client", return_value=mock_http_client
            ):
                ...
                assert mock_shared_cache.mark_seen.call_count == 1, (
                    "Dovrebbe marcare in shared cache"
                )
                ...
                assert mock_http_client.post_sync.call_count == 1, (
                    "Non dovrebbe chiamare API di nuovo"
                )
