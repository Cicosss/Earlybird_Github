...
class TestV16BrazilianTeamExtraction:
    ...
    def test_flamengo_extracted_from_injury_news(self, analyzer):
        ...
        content = (
            "Flamengo enfrenta desfalque importante para o clássico. Atacante sofre lesão muscular."
        )
        result = analyzer.analyze(content)
        assert result.affected_team == "Flamengo", (
            f"Expected 'Flamengo', got '{result.affected_team}'"
        )
    ...
    def test_corinthians_extracted_from_suspension_news(self, analyzer):
        ...
        content = (
            "Corinthians tem baixa confirmada para próxima rodada do Brasileirão por suspensão."
        )
        result = analyzer.analyze(content)
        assert result.affected_team == "Corinthians", (
            f"Expected 'Corinthians', got '{result.affected_team}'"
        )
...
class TestV16HonduranTeamExtraction:
    ...
    def test_motagua_with_lesionados_plural(self, analyzer):
        ...
        result = analyzer.analyze(content)
        assert result.affected_team == "Motagua", (
            f"Expected 'Motagua', got '{result.affected_team}'"
        )
        assert result.category == "INJURY"
    ...
    def test_real_espana_with_descarta(self, analyzer):
        ...
        result = analyzer.analyze(content)
        assert result.affected_team == "Real España", (
            f"Expected 'Real España', got '{result.affected_team}'"
        )
