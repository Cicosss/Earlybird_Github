...
class TestLauncherBackoffReset(unittest.TestCase):
    ...
    def test_no_reset_before_5_minutes(self):
        ...
        self.assertEqual(
            config["restarts"], 3, "Il contatore NON dovrebbe essere resettato prima di 5 minuti"
        )
    ...
    def test_no_reset_if_no_previous_crashes(self):
        ...
        self.assertFalse(
            reset_triggered, "Il reset NON dovrebbe essere triggerato se restarts == 0"
        )
    ...
    def test_no_reset_if_last_start_time_none(self):
        ...
        self.assertEqual(
            config["restarts"], 3, "Il contatore NON dovrebbe cambiare se last_start_time è None"
        )
...
class TestLauncherProcessDiscovery(unittest.TestCase):
    ...
    def test_news_radar_in_candidates(self):
        ...
        self.assertIn("news_radar", keys, "news_radar deve essere presente in PROCESS_CANDIDATES")
    ...
    def test_discover_processes_includes_last_start_time(self):
        ...
        for key, config in processes.items():
            self.assertIn(
                "last_start_time", config, f"Processo {key} deve avere last_start_time nella config"
            )
            self.assertIsNone(
                config["last_start_time"], "last_start_time deve essere None all'inizializzazione"
            )
