...
def test_engine(temp_db):
    ...
    engine = create_engine(
        f"sqlite:///{temp_db}", connect_args={"check_same_thread": False, "timeout": 30}
    )
...
class TestNewsLogCRUD:
    ...
    def test_create_news_log(self, test_session, sample_match_data):
        ...
        # Verify
        saved = (
            test_session.query(NewsLog).filter(NewsLog.match_id == sample_match_data["id"]).first()
        )
        assert saved is not None
...
class TestTelegramChannelTracking:
    ...
    def test_create_telegram_channel(self, test_engine):
        ...
        try:
            ...
            saved = (
                ...
                .filter(TelegramChannel.channel_id == "123456789")
                .first()
            )
    ...
    def test_telegram_message_log(self, test_engine):
        ...
        try:
            ...
            saved = (
                session.query(TelegramMessageLog)
                .filter(TelegramMessageLog.message_id == "msg_001")
                .first()
            )
...
class TestDataFlowE2E:
    ...
    def test_match_ingestion_flow(self, test_session, sample_match_data):
        ...
        home_alias = (
            test_session.query(TeamAlias)
            .filter(TeamAlias.api_name == sample_match_data["home_team"])
            .first()
        )
...
class TestEdgeCases:
    ...
    def test_evaluate_bet_edge_cases(self):
        ...
        from src.analysis.settler import RESULT_PENDING, RESULT_PUSH, evaluate_bet

        # Cancelled match
        result, explanation = evaluate_bet("Home Win", 0, 0, 2.10, "CANCELLED")
        assert result == RESULT_PUSH
        assert "CANCELLED" in explanation.upper() or "Annullata" in explanation

        # Postponed match
        result, explanation = evaluate_bet("Away Win", 0, 0, 3.20, "POSTPONED")
        assert result == RESULT_PUSH

        # Unknown market
        result, explanation = evaluate_bet("Unknown Market XYZ", 2, 1, 2.10, "FINISHED")
        assert result == RESULT_PENDING

        # Empty market string
        result, explanation = evaluate_bet("", 2, 1, 2.10, "FINISHED")
        assert result == RESULT_PENDING

        # None market
        result, explanation = evaluate_bet(None, 2, 1, 2.10, "FINISHED")
        assert result == RESULT_PENDING
    ...
    def test_evaluate_over_under_edge_cases(self):
        ...
        # Stats not available
        result, explanation = evaluate_over_under("Over 9.5 Corners", 10, stat_available=False)
        assert result == RESULT_PENDING

        # Invalid format
        result, explanation = evaluate_over_under("Invalid Format", 10, stat_available=True)
        assert result == RESULT_PENDING

        # Edge case: exactly on the line (should never happen with .5)
        result, explanation = evaluate_over_under("Over 9.5 Corners", 10, stat_available=True)
...
class TestMaintenanceOperations:
    ...
    def test_prune_old_data(self, test_session, sample_match_data):
        ...
        # Get old match IDs
        old_ids = [
            m.id for m in test_session.query(Match).filter(Match.start_time < cutoff_naive).all()
        ]

        # Delete news logs first (FK constraint)
        test_session.query(NewsLog).filter(NewsLog.match_id.in_(old_ids)).delete(
            synchronize_session=False
        )

        # Delete old matches
        test_session.query(Match).filter(Match.id.in_(old_ids)).delete(synchronize_session=False)
...
class TestBotIntegration:
    ...
    def test_biscotto_detection_db_fields(self, test_session, sample_match_data):
        ...
        drop_pct = (
            (saved.opening_draw_odd - saved.current_draw_odd) / saved.opening_draw_odd
        ) * 100
    ...
    def test_case_closed_cooldown(self, test_session, sample_match_data):
        ...
        match = Match(
            ...
            last_deep_dive_time=(datetime.now(timezone.utc) - timedelta(hours=2)).replace(
                tzinfo=None
            ),
        )
        test_session.add(match)
    ...
    def test_alert_threshold_filtering(self, test_session, sample_match_data):
        ...
        settleable = (
            test_session.query(Match).filter(Match.highest_score_sent >= SETTLEMENT_MIN_SCORE).all()
        )
