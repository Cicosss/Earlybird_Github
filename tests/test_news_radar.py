...
from src.utils.content_analysis import ExclusionFilter, RelevanceAnalyzer
...
def test_property_1_config_loading_correctness(sources, global_settings):
    ...
    **Validates: Requirements 1.1, 8.1**
    ...
    try:
        ...
        assert (
            config.global_settings.default_scan_interval_minutes
            == global_settings.default_scan_interval_minutes
        )
...
def test_property_2_circuit_breaker_activation(failure_threshold, num_failures):
    ...
    **Validates: Requirements 1.4**
    ...
    if num_failures >= failure_threshold:
        ...
        assert cb.can_execute() is False, "Should not allow execution when circuit is OPEN"
    else:
        assert cb.state == "CLOSED", (
            f"Expected CLOSED with {num_failures} failures (threshold={failure_threshold})"
        )
        assert cb.can_execute() is True, "Should allow execution when circuit is CLOSED"
...
@given(
    ...
    prefix=st.text(
        min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=("L", "Z"))
    ),
    suffix=st.text(
        min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=("L", "Z"))
    ),
...
from src.services.news_radar import DEEPSEEK_MIN_INTERVAL_SECONDS, DeepSeekFallback
...
@given(
    source_name=st.text(
        min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("L", "N", "Z"))
    ),
    source_url=valid_urls,
    affected_team=st.text(
...
def test_property_8_alert_content_completeness(
    source_name, source_url, affected_team, category, summary, confidence
):
    """
    **Feature: news-radar-monitor, Property 8: Alert Content Completeness**
    **Validates: Requirements 6.1, 6.2**
    ...
    assert expected_category in message, (
        f"category '{expected_category}' (Italian for '{category}') not in message"
    )
...
@settings(max_examples=50)
@given(priorities=st.lists(st.integers(min_value=1, max_value=10), min_size=2, max_size=10))
def test_property_9_priority_based_scan_ordering(priorities):
    ...
    **Validates: Requirements 8.4**
    ...
    for i, priority in enumerate(priorities):
        source = RadarSource(url=f"https://example{i}.com", name=f"Source {i}", priority=priority)
...
async def test_regression_process_content_without_start():
    ...
    result = await monitor._process_content(
        content="Test content with injury news", source=source, url="https://example.com/article"
    )
...
def test_regression_singleton_usage_in_news_radar():
    ...
    async def run_test():
        ...
        from src.services.news_radar import DEFAULT_CACHE_MAX_ENTRIES, ContentCache
...
def test_content_extractor_has_ensure_browser_connected_method():
    ...
    assert callable(extractor._ensure_browser_connected), (
...
def test_content_extractor_has_recreate_browser_method():
    ...
    # Verify it's callable
    assert callable(extractor._recreate_browser_internal), (
...
async def test_extract_with_browser_handles_disconnected_browser():
    ...
    # V1.3: Renamed from _recreate_browser to _recreate_browser_internal
    with patch.object(
        extractor, "_recreate_browser_internal", new_callable=AsyncMock
    ) as mock_recreate:
...
async def test_extract_with_navigation_handles_disconnected_browser():
    ...
    with patch.object(
        ...
        result = await extractor.extract_with_navigation(
            url="https://example.com", link_selector="a.article-link"
        )
...
def test_extract_team_name_finds_team_with_suffix():
    ...
    test_cases = [
        ...
        (
            "Wigan Athletic announce new signing",
            "Wigan Athletic",
        ),  # Not in known_clubs, uses suffix pattern
    ]
...
def test_v14_positive_news_filter_in_imports():
    ...
    from src.utils.content_analysis import PositiveNewsFilter, get_positive_news_filter
...
def test_v14_news_radar_imports_positive_filter():
    """Test that PositiveNewsFilter is available from content_analysis module (V9.5 architecture)."""
    from src.utils.content_analysis import PositiveNewsFilter, get_positive_news_filter
