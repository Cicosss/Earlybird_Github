...
class TestPositionDetection:
    ...
    def test_detect_position_from_player_data(self):
        ...
        assert (
            detect_position_from_player_data({"position": "Goalkeeper"})
            == PlayerPosition.GOALKEEPER
        )
        assert (
            detect_position_from_player_data({"positionDescription": "Central Defender"})
...
class TestRoleEstimation:
    ...
    def test_first_player_is_starter(self):
        ...
        role = estimate_player_role(
            player={"name": "Test"}, group_index=0, player_index_in_group=0, total_in_group=4
        )
    ...
    def test_last_player_is_backup(self):
        ...
        role = estimate_player_role(
            player={"name": "Test"}, group_index=0, player_index_in_group=3, total_in_group=4
        )
    ...
    def test_none_player_returns_backup(self):
        ...
        role = estimate_player_role(
            player=None, group_index=0, player_index_in_group=0, total_in_group=4
        )
        assert role == PlayerRole.BACKUP
...
class TestTeamInjuryImpact:
    ...
    def test_empty_injuries_zero_impact(self):
        ...
        impact = calculate_team_injury_impact(team_name="Test FC", injuries=[], squad_data=None)
    ...
    def test_none_injuries_zero_impact(self):
        ...
        impact = calculate_team_injury_impact(team_name="Test FC", injuries=None, squad_data=None)
        assert impact.total_impact_score == 0.0
    ...
    def test_multiple_injuries_aggregated(self):
        ...
        impact = calculate_team_injury_impact(
            team_name="Test FC", injuries=injuries, squad_data=None
        )
    ...
    def test_severity_critical_with_many_starters(self):
        ...
        impact = calculate_team_injury_impact(
            team_name="Test FC", injuries=injuries, squad_data=squad_data
        )
    ...
    def test_key_players_tracked(self):
        ...
        injuries = [{"name": "Messi", "reason": "Injury"}, {"name": "Unknown", "reason": "Injury"}]
        impact = calculate_team_injury_impact(
            team_name="Test FC",
            injuries=injuries,
            squad_data=None,
...
class TestInjuryDifferential:
    ...
    def test_empty_injuries_both_teams(self):
        ...
        diff = calculate_injury_differential(
            home_team="Home FC", away_team="Away FC", home_injuries=[], away_injuries=[]
        )
    ...
    def test_none_injuries_no_crash(self):
        ...
        diff = calculate_injury_differential(
            home_team="Home FC", away_team="Away FC", home_injuries=None, away_injuries=None
        )
        assert diff.differential == 0.0
...
class TestAnalyzeMatchInjuries:
    ...
    def test_with_context_dicts(self):
        ...
        home_context = {"injuries": [{"name": "Player1", "reason": "Injury"}], "squad": None}
        away_context = {"injuries": [], "squad": None}
    ...
    def test_with_none_context(self):
        ...
        diff = analyze_match_injuries(
            home_team="Home FC", away_team="Away FC", home_context=None, away_context=None
        )
        assert diff.differential == 0.0

    def test_with_key_players(self):
        """Key players vengono considerati."""
        home_context = {"injuries": [{"name": "Messi", "reason": "Injury"}]}
        diff = analyze_match_injuries(
            home_team="Home FC",
            away_team="Away FC",
            home_context=home_context,
...
class TestBuildPlayerInfoMap:
    ...
    def test_build_from_squad_data(self):
        ...
        squad_data = {
            "squad": [
                {"title": "Goalkeepers", "members": [{"name": "Keeper1"}, {"name": "Keeper2"}]},
                {"title": "Defenders", "members": [{"name": "Defender1"}]},
            ]
        }
    ...
    def test_nested_squad_structure(self):
        ...
        squad_data = {
            "squad": {"squad": [{"title": "Forwards", "members": [{"name": "Striker1"}]}]}
        }
        player_map = _build_player_info_map(squad_data)
...
class TestEdgeCases:
    ...
    def test_injury_with_empty_name(self):
        ...
        injuries = [{"name": "", "reason": "Injury"}, {"name": "Valid", "reason": "Injury"}]
        impact = calculate_team_injury_impact(team_name="Test FC", injuries=injuries)
        assert len(impact.players) == 1

    def test_injury_with_none_name(self):
        """Injury con nome None viene saltato."""
        injuries = [{"name": None, "reason": "Injury"}, {"name": "Valid", "reason": "Injury"}]
        impact = calculate_team_injury_impact(team_name="Test FC", injuries=injuries)
        assert len(impact.players) == 1
    ...
    def test_division_by_zero_avoided(self):
        ...
        role = estimate_player_role(
            player={"name": "Test"}, group_index=0, player_index_in_group=0, total_in_group=0
        )
...
class TestIntegrationWithAnalyzer:
    ...
    def test_starter_vs_backup_differential(self):
        ...
        assert diff.home_impact.missing_starters == 2, (
            f"Expected 2 home starters missing, got {diff.home_impact.missing_starters}"
        )
        assert diff.away_impact.missing_starters == 0, (
            f"Expected 0 away starters missing, got {diff.away_impact.missing_starters}"
        )

        # Score adjustment deve favorire away (positivo)
        assert diff.score_adjustment >= 0, (
            f"Expected non-negative score_adjustment, got {diff.score_adjustment}"
        )

        # Severity check
        assert diff.home_impact.severity in ("HIGH", "CRITICAL"), (
            f"Expected HIGH/CRITICAL severity for home, got {diff.home_impact.severity}"
        )
        assert diff.away_impact.severity == "LOW", (
            f"Expected LOW severity for away, got {diff.away_impact.severity}"
        )
    ...
    def test_goalkeeper_absence_high_impact(self):
        ...
        assert diff.home_impact.total_impact_score >= 8.0, (
            f"Expected high impact for GK, got {diff.home_impact.total_impact_score}"
        )
        assert diff.home_impact.defensive_impact >= 8.0, (
            f"Expected high defensive impact, got {diff.home_impact.defensive_impact}"
        )
...
class TestContextAwareAdjustment:
    ...
    def test_edge_case_single_player_group(self):
        ...
        """
        role = estimate_player_role(
            player={"name": "Solo Player"}, group_index=0, player_index_in_group=0, total_in_group=1
        )
        assert role == PlayerRole.STARTER
