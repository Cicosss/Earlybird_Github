...
def test_6_fetch_sources_with_league_filter():
    ...
    try:
        ...
        if sources is not None:
            logger.info(
                f"✅ PASSED: fetch_sources(league_id) returned {len(sources)} sources (from mirror)"
            )
            return True
        else:
...
def test_7_fetch_hierarchical_map():
    ...
    try:
        ...
        if hierarchical_map and "continents" in hierarchical_map:
            logger.info(
                f"✅ PASSED: fetch_hierarchical_map() returned map with {len(hierarchical_map['continents'])} continents"
            )
            return True
        else:
