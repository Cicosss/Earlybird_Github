...
class TestLineupDetectionPatterns:
    ...
    def test_english_confirmed_lineup(self):
        ...
        from src.utils.high_value_detector import SignalType, get_signal_detector
    ...
    def test_spanish_confirmed_lineup(self):
        ...
        from src.utils.high_value_detector import get_signal_detector
...
class TestPositiveNewsFilterSentenceLevel:
    ...
    def test_mixed_news_not_skipped(self):
        ...
        for content in test_cases:
            assert not filter.is_positive_news(content), (
                f"Mixed news should NOT be skipped: {content}"
            )
...
class TestTimezoneAwareScanning:
    ...
    def test_no_timezone_uses_default_interval(self):
        ...
        source = RadarSource(
            url="http://example.com", scan_interval_minutes=5, source_timezone=None
        )
    ...
    def test_invalid_timezone_uses_default(self):
        ...
        source = RadarSource(
            url="http://example.com", scan_interval_minutes=5, source_timezone="Invalid/Timezone"
        )
    ...
    def test_valid_timezone_returns_interval(self):
        ...
        source = RadarSource(
            url="http://example.com", scan_interval_minutes=5, source_timezone="Europe/London"
        )
        ...
        class MockAlert:
            def __init__(self):
                self.enrichment_context = EnrichmentContext(
                    match_id="12345", home_team="Liverpool", away_team="Arsenal"
                )
