...
class TestDixonColesCorrection:
    ...
    def test_correction_clamped_high_lambda(self):
        ...
        correction = MathPredictor.dixon_coles_correction(
            home_goals=0, away_goals=0, home_lambda=4.0, away_lambda=4.0, rho=-0.07
        )
    ...
    def test_correction_normal_case(self):
        ...
        correction = MathPredictor.dixon_coles_correction(
            home_goals=0, away_goals=0, home_lambda=1.5, away_lambda=1.2, rho=-0.07
        )
...
class TestShrinkageKellyRelaxed:
    ...
    def test_kelly_increases_with_sample_size(self):
        ...
        edge_small = MathPredictor.calculate_edge(math_prob=0.60, bookmaker_odd=2.0, sample_size=5)

        edge_large = MathPredictor.calculate_edge(math_prob=0.60, bookmaker_odd=2.0, sample_size=20)
...
class TestInjuryEngineTotalInGroup:
    ...
    def test_zero_total_in_group(self):
        ...
        """
        from src.analysis.injury_impact_engine import PlayerRole, estimate_player_role
        ...
        assert result == PlayerRole.BACKUP, f"Expected BACKUP for total_in_group=0, got {result}"
...
class TestFatigueEngineTimezone:
    ...
    def test_naive_datetime_handled(self):
        ...
        fatigue, matches = calculate_fatigue_index(
            team_schedule=naive_schedule, match_date=naive_match_date, squad_depth_score=1.0
        )
    ...
    def test_mixed_timezone_handled(self):
        ...
        # Should not crash
        fatigue, matches = calculate_fatigue_index(
            team_schedule=mixed_schedule, match_date=aware_match_date, squad_depth_score=1.0
        )
