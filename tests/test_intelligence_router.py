...
def verification_responses(draw):
    ...
    return {
        ...
        "verification_status": draw(
            st.sampled_from(["CONFIRMED", "DENIED", "UNVERIFIED", "OUTDATED"])
        ),
        "confidence_level": draw(st.sampled_from(["HIGH", "MEDIUM", "LOW"])),
        "verification_sources": draw(
            st.lists(st.text(min_size=3, max_size=30), min_size=0, max_size=3)
        ),
        "additional_context": draw(st.text(min_size=0, max_size=100)),
        "betting_impact": draw(
            st.sampled_from(["Critical", "Significant", "Minor", "None", "Unknown"])
        ),
