...
def test_continental_brain():
    ...
    try:
        ...
        if active_leagues:
            ...
            print("⚠️  WARNING: No active leagues found for current UTC hour")
            print(
                f"   This is expected if no continental blocks are active at {current_hour}:00 UTC"
            )
    ...
    try:
        ...
        if cycle_leagues:
            ...
            print("⚠️  WARNING: No leagues found for current cycle")
    except Exception as e:
        print(f"❌ ERROR: {e}")
        import traceback
    ...
    try:
        ...
        if fallback_leagues:
            ...
            print("   Fallback Leagues:")
            for i, league in enumerate(fallback_leagues, 1):
                print(f"     {i}. {league}")
        else:
            print("⚠️  WARNING: No leagues found from mirror fallback")
            print("   Check if data/supabase_mirror.json exists and contains valid data")
    except Exception as e:
        print(f"❌ ERROR: {e}")
        import traceback
    ...
    print("📋 TEST SUMMARY")
    print("=" * 70)
    print("✅ Continental Brain implementation verified")
    print("✅ Supabase sync with active_hours_utc filtering verified")
    print("✅ Fallback to mirror verified")
    print("=" * 70)
