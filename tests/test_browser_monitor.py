...
source_names = st.text(
    min_size=1, max_size=50, alphabet=st.characters(whitelist_categories=("L", "N", "P", "Z"))
)
...
def test_load_config_default_values():
    ...
    config_data = {"sources": [{"url": "https://example.com", "league_key": "soccer_turkey"}]}
...
@settings(max_examples=100)
@given(content1=st.text(min_size=10, max_size=500), content2=st.text(min_size=10, max_size=500))
...
def test_property_3_gemini_response_schema_validity(
    is_relevant, category, affected_team, confidence, summary
):
    """
    **Feature: browser-automation-always-on, Property 3: Gemini Response Schema Validity**
    **Validates: Requirements 3.3**
...
@settings(max_examples=50)
@given(
    is_relevant=st.booleans(), confidence=st.floats(min_value=0.0, max_value=1.0, allow_nan=False)
)
...
def test_property_13_news_item_schema_validity(title, snippet, category, team, confidence):
    ...
    **Validates: Requirements 4.3**
    ...
    results = get_browser_monitor_news(
        match_id="test_match_123", team_names=[team], league_key="soccer_turkey_super_league"
    )
...
def test_property_13_news_item_has_all_fields():
    ...
    **Validates: Requirements 4.3**
    ...
    assert item["confidence"] >= 0.7, "confidence should be >= 0.7 for HIGH confidence"
    assert item["search_type"] == "browser_monitor"
...
def test_property_14_tier_0_priority_boost():
    ...
    **Validates: Requirements 4.4**
    ...
    results = get_browser_monitor_news(
        match_id="match_789", team_names=["Test Team"], league_key="soccer_turkey_super_league"
    )
...
def test_property_12_cooldown_isolation_no_import():
    ...
    **Validates: Requirements 7.4**
    ...
    assert len(cooldown_imports) == 0, (
        f"Browser monitor should not import cooldown modules: {cooldown_imports}"
    )
...
def test_regression_empty_summary_handling():
    ...
    source = MonitoredSource(
        url="https://example.com", league_key="soccer_turkey", name="Test Source"
    )
...
def test_regression_source_name_fallback():
    ...
    source = MonitoredSource(
        url="https://fanatik.com.tr/news/article", league_key="soccer_turkey", name=""
    )
...
def test_news_hunter_browser_monitor_empty_results():
    ...
    results = get_browser_monitor_news(
        match_id="test_match", team_names=["NonExistent Team"], league_key="soccer_unknown_league"
    )
...
def test_news_hunter_browser_monitor_none_team_names():
    ...
    results = get_browser_monitor_news(
        match_id="test_match", team_names=[], league_key="soccer_turkey"
    )
...
def test_browser_monitor_freshness_tag_updates():
    ...
    results = get_browser_monitor_news(
        match_id="match_1", team_names=["Test Team"], league_key="soccer_turkey_super_league"
    )
...
def test_browser_monitor_empty_team_names_handling():
    ...
    results = get_browser_monitor_news(
        match_id="match_1", team_names=[], league_key="soccer_turkey_super_league"
    )
    ...
    results = get_browser_monitor_news(
        match_id="match_2", team_names=[None, ""], league_key="soccer_turkey_super_league"
    )
...
def test_browser_monitor_none_affected_team_handling():
    ...
    results = get_browser_monitor_news(
        match_id="match_1", team_names=["Galatasaray"], league_key="soccer_turkey_super_league"
    )
...
def test_v7_resource_blocking_patterns_defined():
    ...
    image_patterns = [
        p for p in BLOCKED_RESOURCE_PATTERNS if any(ext in p for ext in ["png", "jpg", "gif"])
    ]
    assert len(image_patterns) > 0, "Should block image formats"

    # Check for font patterns
    font_patterns = [
        p for p in BLOCKED_RESOURCE_PATTERNS if any(ext in p for ext in ["woff", "ttf"])
    ]
...
class TestV71CircuitBreaker:
    ...
    def test_circuit_breaker_opens_after_threshold(self):
        ...
        """
        from src.services.browser_monitor import CIRCUIT_BREAKER_FAILURE_THRESHOLD, CircuitBreaker
...
class TestV71BrowserMonitorCircuitBreaker:
    ...
    def test_should_skip_source_when_circuit_open(self):
        ...
        """
        from src.services.browser_monitor import CIRCUIT_BREAKER_FAILURE_THRESHOLD, BrowserMonitor
...
class TestV71RetryWithBackoff:
    ...
    def test_retry_constants_defined(self):
        ...
        from src.services.browser_monitor import MAX_RETRIES, RETRY_BASE_DELAY, RETRY_MAX_DELAY
...
class TestV71Integration:
    ...
    def test_scan_source_uses_circuit_breaker(self):
        ...
        monitor = BrowserMonitor()
        source = MonitoredSource(
            url="https://failing-source.com", league_key="test_league", name="Test Source"
        )
    ...
    def test_news_hunter_integration_with_v71_discovery(self):
        ...
        # Retrieve
        results = get_browser_monitor_news(
            match_id="v71_match", team_names=["Test FC"], league_key="v71_test_league"
        )
...
class TestV72DomainStickyFingerprint:
    ...
    def test_different_domains_can_have_different_profiles(self):
        ...
        from src.utils.browser_fingerprint import BROWSER_PROFILES, BrowserFingerprint
...
class TestV72HttpClientDomainSticky:
    ...
    def test_extract_domain_valid_url(self):
        ...
        assert (
            EarlyBirdHTTPClient._extract_domain("http://news.site.org/article/123")
            == "news.site.org"
        )
        assert (
            EarlyBirdHTTPClient._extract_domain("https://sub.domain.co.uk/") == "sub.domain.co.uk"
        )
    ...
    def test_extract_domain_handles_edge_cases(self):
        ...
        # Invalid URLs
        assert (
            EarlyBirdHTTPClient._extract_domain("not-a-url") is None
            or EarlyBirdHTTPClient._extract_domain("not-a-url") == ""
        )
...
class TestV72HttpExtractionFingerprint:
    ...
    async def test_extract_with_http_uses_domain_fingerprint(self):
        ...
        from src.services.browser_monitor import BrowserMonitor
        from src.utils.browser_fingerprint import get_fingerprint, reset_fingerprint
...
class TestV73BugFixes:
    ...
    async def test_extract_with_retry_returns_tuple(self):
        ...
        with patch.object(
            monitor, "extract_content_hybrid", new_callable=AsyncMock
        ) as mock_extract:
    ...
    async def test_extract_with_retry_network_error_triggers_retry(self):
        ...
        from src.services.browser_monitor import MAX_RETRIES, BrowserMonitor
        ...
        with patch.object(
            monitor, "extract_content_hybrid", side_effect=mock_extract_network_error
        ):
    ...
    def test_get_memory_usage_uses_module_import(self):
        ...
        """
        from src.services.browser_monitor import PSUTIL_AVAILABLE, get_memory_usage_percent
...
class TestV731EdgeCasesComprehensive:
    ...
    async def test_scan_source_with_string_confidence_from_ai(self):
        ...
        monitor = BrowserMonitor()
        source = MonitoredSource(
            url="https://test.com/article", league_key="soccer_test", name="Test Source"
        )
...
class TestV74PaginatedNavigation:
    ...
    def test_monitored_source_default_navigation_mode(self):
        ...
        source = MonitoredSource(url="https://test.com", league_key="soccer_test")
    ...
    def test_monitored_source_max_links_default(self):
        ...
        from src.services.browser_monitor import DEFAULT_MAX_LINKS_PER_PAGINATED, MonitoredSource
...
class TestV74ExtractWithNavigation:
    ...
    async def test_extract_with_navigation_returns_empty_without_browser(self):
        ...
        results = await monitor.extract_with_navigation(
            url="https://test.com", link_selector="a.article"
        )
...
class TestV74AnalyzeAndCreateNews:
    ...
    async def test_analyze_and_create_news_handles_cached_content(self):
        ...
        news = await monitor._analyze_and_create_news(
            source, "https://test.com/article", "Cached content"
        )
...
class TestV74EdgeCases:
    ...
    def test_monitored_source_max_links_custom_value(self):
        ...
        source = MonitoredSource(url="https://test.com", league_key="soccer_test", max_links=10)
    ...
    async def test_extract_with_navigation_deduplicates_links(self):
        ...
        with patch.object(monitor, "extract_content_hybrid", side_effect=mock_extract):
            with patch("asyncio.sleep", new_callable=AsyncMock):
                results = await monitor.extract_with_navigation(
                    url="https://test.com", link_selector="a.article", max_links=5
                )
        ...
        for source in config.sources:
            if source.navigation_mode == "paginated":
                assert source.link_selector, (
                    f"Source {source.name} is paginated but has no link_selector"
                )
...
class TestV75ContentAnalysisModule:
    ...
    def test_relevance_analyzer_detects_injury(self):
        ...
        result = ra.analyze(
            "Manchester United star injured, ruled out for 3 weeks with hamstring strain"
        )
    ...
    def test_relevance_analyzer_detects_youth_callup(self):
        ...
        result = ra.analyze(
            "Primavera player promoted to first team, youth academy talent called up"
        )
...
class TestV76RegressionBugFixes:
    ...
    def test_empty_category_becomes_other(self):
        ...
        assert category == "OTHER", f"Empty category should become 'OTHER', got: {category}"
...
class TestV76NewsRadarSafeAccess:
    ...
    def test_choices_invalid_type_handled(self):
        ...
        for response in invalid_responses:
            ...
            assert not is_valid, f"Invalid choice type should be detected: {type(first_choice)}"

    def test_valid_choices_structure_accepted(self):
        """Valid choices structure should be accepted."""
        response = {"choices": [{"message": {"content": '{"is_relevant": true}'}}]}
...
class TestV76RecordSourceFailureDefault:
    ...
    def test_record_source_failure_default_is_false(self):
        ...
        assert "is_network_error" in params, "is_network_error parameter missing"
    ...
    def test_record_source_failure_without_flag_does_not_count(self):
        ...
        cb = monitor._get_circuit_breaker(url)
        assert cb.failure_count == 0, "REGRESSION: Failure counted without is_network_error=True"
    ...
    def test_record_source_failure_with_flag_counts(self):
        ...
        cb = monitor._get_circuit_breaker(url)
        assert cb.failure_count == 1, "Failure should be counted with is_network_error=True"
...
class TestV76BrowserAutoRecovery:
    ...
    async def test_regression_browser_disconnected_recovery(self):
        ...
        mock_page.content = AsyncMock(
            return_value="<html><body>Test content with enough characters to pass validation</body></html>"
        )
        mock_page.inner_text = AsyncMock(
            return_value="Test content with enough characters to pass validation"
        )
        mock_page.close = AsyncMock()
    ...
    async def test_extract_content_recovers_from_disconnected_browser(self):
        ...
        mock_page.goto = AsyncMock()
        mock_page.content = AsyncMock(
            return_value="<html><body><article>This is test content with more than 100 characters to pass the minimum length validation check.</article></body></html>"
        )
        mock_page.inner_text = AsyncMock(
            return_value="This is test content with more than 100 characters to pass the minimum length validation check."
        )
        mock_page.close = AsyncMock()
