...
def is_valid_html(html: str | None) -> bool:
    """
    ...
    has_content = any(
        indicator.lower() in html_stripped.lower() for indicator in CONTENT_INDICATORS
    )
    if not has_content:
    ...
    for pattern in EMPTY_CONTENT_PATTERNS:
        if re.search(pattern, html_stripped, re.IGNORECASE):
            logger.debug("[TRAFILATURA] Empty/error content pattern detected")
            return False
...
def extract_with_trafilatura(
    ...
    try:
        ...
        logger.debug(
            f"[TRAFILATURA] Extraction returned insufficient content: {len(text) if text else 0} chars"
        )
        return None
...
def _extract_with_regex(html: str) -> str | None:
    """
        ...
        for pattern in content_patterns:
            ...
            if match:
                ...
                if text and len(text) >= MIN_EXTRACTED_LENGTH:
                    logger.debug(
                        f"[TRAFILATURA] Regex extraction succeeded with pattern: {pattern[:30]}..."
                    )
                    return text
...
def _extract_raw_text(html: str) -> str | None:
    """
