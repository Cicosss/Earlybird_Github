...
def parse_relative_time(time_str: str) -> int | None:
    ...
    patterns = [
        (
            r"(\d+)\s*(?:minute|min|minuto|minuti|minuten|minut)s?\s*(?:ago|fa|hace|atrás|vor|il y a)?",
            1,
        ),
        (
            r"(\d+)\s*(?:hour|hr|ora|ore|hora|horas|stunde|stunden|heure)s?\s*(?:ago|fa|hace|atrás|vor|il y a)?",
            60,
        ),
        (
            r"(\d+)\s*(?:day|giorno|giorni|día|dias|tag|tage|jour)s?\s*(?:ago|fa|hace|atrás|vor|il y a)?",
            60 * 24,
        ),
        (
            r"(\d+)\s*(?:week|settimana|settimane|semana|semanas|woche|wochen|semaine)s?\s*(?:ago|fa|hace|atrás|vor|il y a)?",
            60 * 24 * 7,
        ),
    ]
...
def get_league_decay_rate(league_key: str | None) -> float:
    """
