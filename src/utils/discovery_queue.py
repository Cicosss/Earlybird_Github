...
class DiscoveryQueue:
    ...
    def __init__(self, max_entries: int = DEFAULT_MAX_ENTRIES, ttl_hours: int = DEFAULT_TTL_HOURS):
        ...
        self._high_priority_callback: Callable[[str], None] | None = None
    ...
    def push(
        ...
        with self._lock:
            ...
            should_trigger = (
                self._high_priority_callback is not None
                and confidence >= self._high_priority_threshold
                and category in self._high_priority_categories
            )
        ...
        if callback_ref is not None:
            try:
                logger.info(
                    f"🚨 [QUEUE] High-priority discovery! Triggering callback for {league_key} (conf={confidence:.2f}, cat={category})"
                )
        ...
        logger.debug(
            f"📥 [QUEUE] Pushed discovery: {title[:50] if title else 'No title'} for {team}"
        )
        return item_uuid

    def pop_for_match(
        self, match_id: str, team_names: list[str], league_key: str
    ) -> list[dict[str, Any]]:
        """
        ...
        if results:
            logger.info(
                f"📤 [QUEUE] Retrieved {len(results)} discoveries for {team_names[0] if team_names else 'unknown'}"
            )
    ...
    def clear(self, league_key: str | None = None) -> int:
        """
    ...
    def size(self, league_key: str | None = None) -> int:
        """
...
_discovery_queue: DiscoveryQueue | None = None
_queue_lock = Lock()
