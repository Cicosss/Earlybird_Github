...
def validate_dict_has_keys(
    value: Any, field_name: str, required_keys: list[str]
) -> ValidationResult:
...
def validate_news_item(
    item: dict[str, Any], strict: bool = False, allow_null_match_id: bool = False
) -> ValidationResult:
    """
    ...
    if strict:
        ...
        if "minutes_old" in item:
            age_result = validate_positive_number(
                item["minutes_old"], "minutes_old", allow_zero=True
            )
            if not age_result:
...
def validate_verification_request(request: Any) -> ValidationResult:
    ...
    data = (
        ...
        else (request.to_dict() if hasattr(request, "to_dict") else request.__dict__)
    )
