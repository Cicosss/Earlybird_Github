...
class SharedContentCache:
            ...
            if content:
                ...
                if content_hash and content_hash in self._content_cache:
                    ...
                    if now - cached_time <= timedelta(hours=self._ttl_hours):
                        ...
                        logger.debug(
                            f"🔄 [SHARED-CACHE] Content duplicate detected (original: {cached_source})"
                        )
                        return True
                    else:
                        # Expired, remove it
            ...
            if url:
                ...
                if normalized_url and normalized_url in self._url_cache:
                    ...
                    if now - cached_time <= timedelta(hours=self._ttl_hours):
                        ...
                        self._stats[source]["duplicates"] += 1
                        logger.debug(
                            f"🔄 [SHARED-CACHE] URL duplicate detected (original: {cached_source})"
                        )
                        return True
                    else:
                        # Expired, remove it
    ...
    def cleanup_expired(self) -> int:
        ...
        with self._lock:
            ...
            expired_content = [
                h
                for h, (ts, _) in self._content_cache.items()
            ...
            expired_urls = [
                u
                for u, (ts, _) in self._url_cache.items()
                if now - ts > timedelta(hours=self._ttl_hours)
            ]
            ...
            expired_simhash = [
                sh
                for sh, (ts, _, _) in self._simhash_cache.items()
                if now - ts > timedelta(hours=self._ttl_hours)
            ]
