...
class BrowserFingerprint:
    ...
    def get_headers_for_domain(self, domain: str) -> dict[str, str]:
        ...
        with self._lock:
            ...
            if domain not in self._domain_profiles:
                ...
                logger.debug(
                    f"🎭 Domain {domain} assigned profile: {self._domain_profiles[domain].name}"
                )
    ...
    def force_rotate_domain(self, domain: str) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"🔄 Domain {domain} fingerprint rotated: {old_name} → {self._domain_profiles[domain].name}"
            )
...
def get_fingerprint() -> BrowserFingerprint:
    ...
    with _fingerprint_lock:
        if _fingerprint_instance is None:
            ...
            logger.info(
                f"🎭 BrowserFingerprint singleton created ({len(BROWSER_PROFILES)} profiles)"
            )
