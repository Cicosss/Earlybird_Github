...
def fuzzy_match_team(query: str, text: str, threshold: int = 75) -> tuple[bool, int]:
...
def fuzzy_match_player(
    player_name: str, text: str, threshold: int = 70
...
def get_multilang_form_pattern() -> str:
    ...
    return (
        rf"({won_patterns})\s*(\d+)[^.]*?({drew_patterns})\s*(\d+)[^.]*?({lost_patterns})\s*(\d+)"
    )
