#!/usr/bin/env python3
"""
Isolated Scrapling Pilot Test with Live Supabase Data
======================================================

This script tests the Scrapling integration in NitterPool using real data
from the Supabase social_sources table. It fetches the first 2 social handles
and attempts to scrape their tweets using the new Scrapling-powered fetch_tweets_async().
...
async def test_single_handle(nitter_pool: NitterPool, handle: str, source_name: str) -> dict:
    ...
    try:
        ...
        # Fetch tweets using the new Scrapling-powered method
        tweets = await nitter_pool.fetch_tweets_async(handle, max_retries=3)
...
async def main():
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("🚀 ISOLATED SCRAPLING PILOT TEST - LIVE SUPABASE DATA")
    logger.info("=" * 80 + "\n")

    # Step 1: Initialize Supabase Provider
    logger.info("📡 Step 1: Initializing Supabase Provider...")
    supabase = SupabaseProvider()

    if not supabase.is_connected():
        logger.error("❌ Failed to connect to Supabase")
        logger.error(f"Connection error: {supabase.get_connection_error()}")
        logger.info("\n⚠️ Falling back to local mirror...")
        # We'll continue anyway as the mirror might have data

    # Step 2: Fetch first 2 social handles from Supabase
    logger.info("📡 Step 2: Fetching first 2 social handles from Supabase...")
    social_sources = supabase.get_social_sources()

    if not social_sources:
        logger.error("❌ No social sources found in Supabase or local mirror")
        return
    ...
    # Step 3: Initialize NitterPool with Scrapling
    logger.info("\n🔧 Step 3: Initializing NitterPool with Scrapling...")
    nitter_pool = NitterPool()
    logger.info("✅ NitterPool initialized with Scrapling stealth capabilities")

    # Step 4: Test each handle
    logger.info("\n🧪 Step 4: Testing Scrapling against real targets...")
    results = []
