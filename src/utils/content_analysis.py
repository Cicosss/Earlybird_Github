...
class PositiveNewsFilter:
    ...
    def __init__(self):
        ...
        positive_pattern = (
            r"\b(" + "|".join(re.escape(kw) for kw in self.POSITIVE_KEYWORDS) + r")\b"
        )
        self._positive_pattern = re.compile(positive_pattern, re.IGNORECASE)

        # V1.5: Compile negative override pattern
        negative_pattern = (
            r"\b(" + "|".join(re.escape(kw) for kw in self.NEGATIVE_OVERRIDE_KEYWORDS) + r")\b"
        )
    ...
    def get_positive_reason(self, content: str) -> str | None:
        """
...
class RelevanceAnalyzer:
    ...
    def _extract_team_name(self, content: str) -> str | None:
        """
