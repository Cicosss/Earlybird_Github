...
from config.settings import DEFAULT_ODDS_GOALS, SETTLEMENT_MIN_SCORE
...
class SettlementService:
    ...
    def run_settlement(self, lookback_hours: int = 48) -> dict:
        ...
        logger.info(
            f"📊 SETTLEMENT COMPLETE: {stats['wins']}W / {stats['losses']}L | ROI: {stats['roi_pct']:.1f}%"
        )
    ...
    def _query_matches_to_settle(self, lookback_hours: int, stats: dict) -> list[dict]:
        ...
        with get_db_context() as db:
            try:
                ...
                matches = (
                    db.query(Match)
                    .options(joinedload(Match.news_logs))
                    .filter(
                        and_(
    ...
    def _fetch_match_results(self, matches_to_settle: list[dict], stats: dict) -> list[dict]:
        ...
        for match_data in matches_to_settle:
            ...
            result = self._get_match_result(
                match_data["home_team"], match_data["away_team"], match_data["start_time"]
            )

            if not result:
                stats["pending"] += 1
                logger.debug(
                    f"Result not available for {match_data['home_team']} vs {match_data['away_team']}"
                )
                continue
            ...
            if match_data["combo_suggestion"]:
                ...
                logger.info(
                    f"🧩 [SETTLEMENT] Combo evaluation: {match_data['combo_suggestion']} → {combo_outcome}"
                )
            ...
            try:
                for item in results_cache:
                    ...
                    if outcome == RESULT_PUSH:
                        ...
                        total_return += stake
                        logger.info(
                            f"⚠️ VOID: {match_data['home_team']} vs {match_data['away_team']} | {match_data['recommended_market']} | {explanation}"
                        )
                    ...
                    if outcome == RESULT_WIN:
                        ...
                        logger.info(
                            f"✅ WIN: {match_data['home_team']} vs {match_data['away_team']} | {match_data['recommended_market']} @{bet_odds:.2f} | {explanation}"
                        )
                    else:
                        stats["losses"] += 1
                        logger.info(
                            f"❌ LOSS: {match_data['home_team']} vs {match_data['away_team']} | {match_data['recommended_market']} @{bet_odds:.2f} | {explanation}"
                        )
    ...
    def _calculate_final_statistics(self, stats: dict) -> None:
        ...
        if clv_values:
            ...
            stats["clv_positive_rate"] = round(
                len([c for c in clv_values if c > 0]) / len(clv_values) * 100, 1
            )
            logger.info(
                f"📈 CLV SUMMARY: Avg {stats['avg_clv']:+.2f}% | Positive CLV: {stats['clv_positive_rate']}% ({len(clv_values)} bets tracked)"
            )
        ...
        if combo_bets:
            ...
            stats["combo_stats"] = {
                ...
                "combo_win_rate": round((combo_wins / (combo_wins + combo_losses)) * 100, 1)
                if (combo_wins + combo_losses) > 0
                else 0,
            }
            ...
            logger.info(
                f"🧩 COMBO STATS: {combo_wins}W / {combo_losses}L / {combo_pending}P | Win Rate: {stats['combo_stats']['combo_win_rate']}%"
            )
            for exp_type, data in expansion_performance.items():
                logger.info(
                    f"   {exp_type}: {data['wins']}W/{data['losses']}L ({data['win_rate']}%)"
                )
    ...
    def _get_bet_odds(self, match_data: dict) -> float:
        ...
        # Priority 3: Use current odds (fallback)
            ...
            if "home" in market_lower and "win" in market_lower:
                bet_odds = (
                    match_data["current_home_odd"]
                    if match_data["current_home_odd"] and match_data["current_home_odd"] > 1.0
                    else None
                )
            elif "away" in market_lower and "win" in market_lower:
                bet_odds = (
                    match_data["current_away_odd"]
                    if match_data["current_away_odd"] and match_data["current_away_odd"] > 1.0
                    else None
                )
            elif "draw" in market_lower:
                bet_odds = (
                    match_data["current_draw_odd"]
                    if match_data["current_draw_odd"] and match_data["current_draw_odd"] > 1.0
                    else None
                )
    ...
    def _calculate_clv_for_bet(self, match_data: dict, bet_odds: float) -> float | None:
        """
        ...
        closing_odds = (
            match_data.get("odds_at_kickoff") or match_data.get("closing_odds") or bet_odds
        )  # Prefer new field
    ...
    def _get_match_result(
        ...
        try:
            ...
            for match in previous:
                ...
                if (
                    home_team.lower() in match_home.lower()
                    or match_home.lower() in home_team.lower()
                ) and (
                    away_team.lower() in match_away.lower()
                    or match_away.lower() in away_team.lower()
                ):
                    ...
                    if isinstance(match_status, dict):
                        ...
                        if (
                            is_cancelled
                            or "postponed" in reason
                            or "cancelled" in reason
                            or "abandoned" in reason
                        ):
                            status_type = "POSTPONED" if "postponed" in reason else "CANCELLED"
                            logger.info(
                                f"⚠️ Match rinviato/annullato: {match_home} vs {match_away} ({reason or status_type})"
                            )
                            return {
                                "home_score": 0,
                                "away_score": 0,
    ...
    def _evaluate_bet(
        ...
        if home_score is None or away_score is None:
            logger.warning(
                f"⚠️ evaluate_bet called with None score: home={home_score}, away={away_score}"
            )
        ...
        if "corner" in market_lower:
            ...
            if home_corners is None or away_corners is None:
                return RESULT_PENDING, "⏳ Corner stats non disponibili"
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Corner stats non numerici: home={home_corners}, away={away_corners}"
                )
                return RESULT_PENDING, "⏳ Corner stats non validi"
        ...
        if "card" in market_lower:
            ...
            if (
                match_stats.get("home_yellow_cards") is None
                and match_stats.get("away_yellow_cards") is None
            ):
    ...
    def _evaluate_over_under(
        self, market_string: str, actual_total: int, stat_available: bool = True
    ) -> tuple[str, str]:
        """
        ...
        if direction == "over":
            if actual_total > limit:
                return (
                    RESULT_WIN,
                    f"✅ Over {limit} {stat_type.title()}s Preso ({actual_total} totali)",
                )
            else:
            ...
            if actual_total < limit:
                return (
                    RESULT_WIN,
                    f"✅ Under {limit} {stat_type.title()}s Preso ({actual_total} totali)",
                )
            else:
    ...
    def _evaluate_combo_bet(
        ...
        if len(components) != 2:
            return (
                RESULT_PENDING,
                f"Combo con {len(components)} componenti non supportata",
                "unknown",
            )
        ...
        for i, component in enumerate(components):
            outcome, explanation = self._evaluate_bet(
                component, home_score, away_score, None, "FINISHED", match_stats
            )
        ...
        if all(o == RESULT_WIN for o in outcomes):
            ...
            return (
                RESULT_PENDING,
                f"⏳ COMBO IN ATTESA | {' | '.join(explanations)}",
                expansion_type,
            )
        else:
            return (
                RESULT_PENDING,
                f"⏳ COMBO STATO SCONOSCIUTO | {' | '.join(explanations)}",
                expansion_type,
            )
