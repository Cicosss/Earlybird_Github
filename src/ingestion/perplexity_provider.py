...
from src.prompts.system_prompts import BETTING_STATS_SYSTEM_PROMPT, DEEP_DIVE_SYSTEM_PROMPT
...
class PerplexityProvider:
    ...
    def get_match_deep_dive(
        ...
        try:
            ...
            if result:
                logger.info("✅ [PERPLEXITY] Deep dive complete")
                return result
            else:
    ...
    def _query_api(self, prompt: str, task_type: str = "deep_dive") -> dict | None:
        ...
        try:
            response = requests.post(
                PERPLEXITY_API_URL, headers=headers, json=payload, timeout=PERPLEXITY_TIMEOUT
            )

            if response.status_code != 200:
                logger.warning(
                    f"⚠️ [PERPLEXITY] API error: {response.status_code} - {response.text[:200]}"
                )
                return None
    ...
    def verify_news_batch(
        ...
        if verified_count > 0:
            logger.info(
                f"✅ [PERPLEXITY] Verified {verified_count}/{len(items_to_verify)} news items"
            )
    ...
    def get_betting_stats(
        ...
        logger.info(
            f"🔮 [PERPLEXITY] Fetching betting stats: {home_team} vs {away_team} ({match_date})"
        )
    ...
    def confirm_biscotto(
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"✅ [PERPLEXITY] Biscotto confirmation: confirmed={confirmed}, boost=+{boost}, rec={recommendation}"
                )
                return normalized
            else:
