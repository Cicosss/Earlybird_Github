...
class BudgetManager:
    ...
    def __init__(
        ...
        self._allocations = allocations or TAVILY_BUDGET_ALLOCATION
        ...
        logger.info(
            f"📊 BudgetManager initialized: {monthly_limit} calls/month, "
    ...
    def can_call(self, component: str, is_critical: bool = False) -> bool:
        ...
        if usage_pct >= TAVILY_DISABLED_THRESHOLD:
            if is_critical or component in self._critical_components:
                logger.debug(f"📊 [BUDGET] Critical call allowed for {component} in disabled mode")
                return True
            logger.warning(f"⚠️ [BUDGET] Call blocked for {component}: budget disabled (>{TAVILY_DISABLED_THRESHOLD*100:.0f}%)")
            return False

        # Degraded mode (>90%): Throttle non-critical
        if usage_pct >= TAVILY_DEGRADED_THRESHOLD:
            if is_critical or component in self._critical_components:
                return True
            ...
            if component_used >= component_limit * 0.5:
                logger.warning(f"⚠️ [BUDGET] Call throttled for {component}: degraded mode")
                return False
        ...
        if component_limit > 0 and component_used >= component_limit:
            logger.warning(f"⚠️ [BUDGET] Component {component} at allocation limit ({component_limit})")
            return False
    ...
    def record_call(self, component: str) -> None:
        ...
        if self._monthly_used % 100 == 0:
            logger.info(f"📊 [BUDGET] Usage: {self._monthly_used}/{self._monthly_limit} ({usage_pct:.1f}%)")
    ...
    def get_status(self) -> BudgetStatus:
        ...
        return BudgetStatus(
            ...
            is_degraded=usage_pct >= TAVILY_DEGRADED_THRESHOLD,
            is_disabled=usage_pct >= TAVILY_DISABLED_THRESHOLD,
    ...
    def reset_monthly(self) -> None:
        ...
        logger.info("📊 [BUDGET] Monthly reset complete")
    ...
    def _check_daily_reset(self) -> None:
        ...
        if self._last_reset_day is None:
            ...
            self._daily_used = 0
            self._last_reset_day = current_day
            logger.debug("📊 [BUDGET] Daily counter reset")
    ...
    def _check_thresholds(self) -> None:
        ...
        disabled_count = int(self._monthly_limit * TAVILY_DISABLED_THRESHOLD)
        if self._monthly_used == disabled_count:
            logger.warning(
                f"🚨 [BUDGET] DISABLED threshold reached ({TAVILY_DISABLED_THRESHOLD*100:.0f}%): "
                f"Only critical calls allowed"
            )

        # Check 90% threshold (degraded)
        degraded_count = int(self._monthly_limit * TAVILY_DEGRADED_THRESHOLD)
        if self._monthly_used == degraded_count:
            logger.warning(
                f"⚠️ [BUDGET] DEGRADED threshold reached ({TAVILY_DEGRADED_THRESHOLD*100:.0f}%): "
