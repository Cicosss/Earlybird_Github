...
# ============================================
try:
    from src.analysis.market_intelligence import init_market_intelligence_db, save_odds_snapshot
...
def should_update_league(db, sport_key: str) -> tuple[bool, str, float | None]:
    ...
    if hours_to_match <= HIGH_ALERT_THRESHOLD:
        if hours_since_update >= 1:
            ...
            return (
                False,
                f"HIGH_ALERT_FRESH (updated {hours_since_update:.1f}h ago)",
                hours_to_match,
            )
...
def extract_totals_odds(bookmakers_data: list) -> tuple[float | None, float | None]:
    """
...
def extract_sharp_odds_analysis(
    ...
    if result["sharp_home"] and result["avg_home"]:
        ...
        if diff > 0.10:
            result["home_signal"] = (
                f"SHARP DROP: Sharp {result['sharp_home']:.2f} vs Avg {result['avg_home']:.2f} (diff: {diff:.2f})"
            )
    ...
    if result["sharp_draw"] and result["avg_draw"]:
        ...
        if diff > 0.10:
            result["draw_signal"] = (
                f"SHARP DROP: Sharp {result['sharp_draw']:.2f} vs Avg {result['avg_draw']:.2f} (diff: {diff:.2f})"
            )
    ...
    if result["sharp_away"] and result["avg_away"]:
        ...
        if diff > 0.10:
            result["away_signal"] = (
                f"SHARP DROP: Sharp {result['sharp_away']:.2f} vs Avg {result['avg_away']:.2f} (diff: {diff:.2f})"
            )
