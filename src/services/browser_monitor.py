...
class CircuitBreaker:
    ...
    def __init__(
        ...
        self._lock = threading.Lock()  # V7.8: Thread-safe circuit breaker state
...
class MonitoredSource:
    ...
    link_selector: str | None = None  # V7.4: CSS selector for paginated mode
...
class ContentCache:
    ...
    def __init__(
        ...
        self._lock = threading.Lock()  # V7.8: Thread-safe cache operations
...
def load_config(config_file: str = DEFAULT_CONFIG_FILE) -> MonitorConfig:
    ...
    try:
        ...
        global_settings = GlobalSettings(
            ...
            navigation_interval_seconds=gs_data.get(
                "navigation_interval_seconds", DEFAULT_NAVIGATION_INTERVAL_SECONDS
            ),
...
class BrowserMonitor:
    ...
    def __init__(
        ...
        on_news_discovered: Callable[[DiscoveredNews], None] | None = None,
    ):
        """
        ...
        self._browser_lock: asyncio.Lock | None = None  # Initialized in start()

        # V7.8: Lock per proteggere l'accesso thread-safe alle statistiche
        # Il BrowserMonitor gira in un thread separato, ma il main thread legge le stats
    ...
    async def _ensure_browser_connected(self) -> bool:
        ...
        async with self._browser_lock:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ [BROWSER-MONITOR] Browser state check failed: {e}, recreating..."
                )
                return await self._recreate_browser_internal()
    ...
    async def extract_content(self, url: str) -> str | None:
        ...
        for attempt in range(max_retries):
            ...
            if not await self._ensure_browser_connected():
                logger.error(
                    "❌ [BROWSER-MONITOR] Browser not available and could not be recreated"
                )
                return None
            ...
            async with self._page_semaphore:
                ...
                try:
                    ...
                    if not text:
                        ...
                        logger.debug(
                            f"📄 [BROWSER-MONITOR] Using fallback extraction for {url[:40]}..."
                        )
    ...
    async def extract_content_hybrid(self, url: str) -> str | None:
        """
        ...
        try:
            ...
            logger.debug(
                f"🔄 [BROWSER-MONITOR] HTTP failed, falling back to browser: {url[:40]}..."
            )
        ...
        for attempt in range(max_retries):
            try:
                async with self._page_semaphore:
                    ...
                    logger.info(
                        f"🔗 [BROWSER-MONITOR] Found {len(unique_links)} links on {url[:40]}..."
                    )
                ...
                if "closed" in error_msg or "target" in error_msg:
                    logger.warning(
                        f"⚠️ [BROWSER-MONITOR] Browser disconnected during paginated navigation: {e}"
                    )
    ...
    async def _extract_with_retry(self, url: str) -> tuple[str | None, bool]:
        """
    ...
    async def _analyze_and_create_news(
        ...
        if len(content) < TAVILY_SHORT_CONTENT_THRESHOLD:
            ...
            if expanded_content:
                ...
                logger.debug(
                    f"🔍 [BROWSER-MONITOR] Content expanded with Tavily: {len(content)} chars"
                )
        ...
        if local_result.confidence >= ALERT_CONFIDENCE_THRESHOLD:
            ...
            if not analysis:
                ...
                logger.debug(
                    f"⚠️ [BROWSER-MONITOR] DeepSeek returned None for: {article_url[:50]}..."
                )
                return None
    ...
    async def _tavily_expand_short_content(self, content: str, url: str) -> str | None:
        """
    ...
    async def analyze_relevance(self, content: str, league_key: str) -> dict[str, Any] | None:
        """
    ...
    async def _analyze_with_deepseek(
        self, content: str, league_key: str, timeout: int = 30, max_retries: int = 3
    ) -> dict[str, Any] | None:
        """
        ...
        while retry_count <= max_retries:
            try:
                ...
                if not choices or not isinstance(choices, list) or len(choices) == 0:
                    logger.warning(
                        f"⚠️ [BROWSER-MONITOR] DeepSeek response missing 'choices': {data}"
                    )
                ...
                if not isinstance(first_choice, dict):
                    logger.warning(
                        f"⚠️ [BROWSER-MONITOR] DeepSeek invalid choice format: {first_choice}"
                    )
                ...
                if not isinstance(message, dict):
                    logger.warning(
                        f"⚠️ [BROWSER-MONITOR] DeepSeek invalid message format: {first_choice}"
                    )
                ...
                logger.error(
                    f"❌ [BROWSER-MONITOR] DeepSeek unexpected error: {type(e).__name__}: {e}"
                )
                last_error = e
                break
...
- summary should be concise and informative"""
    ...
    def _parse_relevance_response(self, response_text: str) -> dict[str, Any] | None:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._stats_lock:
            ...
            api_calls_saved = (
                self._excluded_count + self._skipped_low_confidence + self._direct_alerts
            )
            api_savings_percent = (
                (api_calls_saved / total_analyzed * 100) if total_analyzed > 0 else 0.0
            )
            ...
            return {
                ...
                "last_cycle_time": self._last_cycle_time.isoformat()
...
_browser_monitor_instance: BrowserMonitor | None = None
