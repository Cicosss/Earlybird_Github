...
class MathPredictor:
    ...
    def __init__(self, league_avg: float = DEFAULT_LEAGUE_AVG, league_key: Optional[str] = None):
        ...
        if league_key and self.home_advantage != 0.30:  # Log only if non-default
            logger.debug(
                f"MathPredictor initialized for {league_key} with HA={self.home_advantage}"
            )
    ...
    def calculate_strength(
        self, home_scored: float, home_conceded: float, away_scored: float, away_conceded: float
    ...
    def simulate_match(
        ...
        # Validate inputs
        if any(
            x is None or x < 0 for x in [home_scored, home_conceded, away_scored, away_conceded]
        ):
        ...
        for home_goals in range(max_goals + 1):
            for away_goals in range(max_goals + 1):
                ...
                prob = self.poisson_probability(home_lambda, home_goals) * self.poisson_probability(
    ...
    def analyze_match(
        ...
        poisson = self.simulate_match(home_scored, home_conceded, away_scored, away_conceded)
...
def format_math_context(analysis: dict, market: str = "home") -> str:
    ...
    if market in edges:
        ...
        if edge.has_value:
            lines.append(
                f"   🎯 **VALUE DETECTED on {edge.market}:** Math {edge.math_prob:.1f}% vs Implied {edge.implied_prob:.1f}% = **+{edge.edge:.1f}% Edge**"
            )
            lines.append(
                f"   💰 Fair Odd: {edge.fair_odd} | Actual: {edge.actual_odd} | Kelly Stake: {edge.kelly_stake}%"
            )
        else:
            lines.append(
                f"   ⚠️ No value on {edge.market}: Math {edge.math_prob:.1f}% vs Implied {edge.implied_prob:.1f}% = {edge.edge:.1f}%"
            )
...
def calculate_btts_trend(h2h_matches: list) -> dict:
    ...
    """
    result = {"btts_rate": 0.0, "btts_hits": 0, "total_games": 0, "trend_signal": "Unknown"}

    # Safety: Handle None or non-list input
    if not h2h_matches or not isinstance(h2h_matches, list):
        logger.debug(
            f"calculate_btts_trend: Empty or invalid input (type={type(h2h_matches).__name__})"
        )
        return result
    ...
    logger.info(
        f"⚽ H2H BTTS Trend: {btts_hits}/{valid_games} games ({result['btts_rate']}%) - {result['trend_signal']}"
    )
...
if __name__ == "__main__":
    ...
    for market, edge in result["edges"].items():
        ...
        print(
            f"   {market.upper()}: {status} | Edge: {edge.edge:+.1f}% | Kelly: {edge.kelly_stake}%"
        )
