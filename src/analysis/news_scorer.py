...
from src.processing.sources_config import SourceTier, get_trust_score
...
def _score_freshness(date_str: str) -> tuple[float, float | None]:
    """
...
def score_news_item(news_item: dict[str, Any]) -> NewsScore:
    ...
    primary_driver = _determine_primary_driver(
        source_points, content_points, freshness_points, specificity_points, source_info.source_type
    )
...
def format_batch_score_for_prompt(batch_result: dict[str, Any]) -> str:
    ...
    if top:
        score = top["score"]
        lines.append(
            f"- Best source: TIER {score.source_tier} ({score.source_type}) - {score.raw_score:.1f}/10"
        )
...
if __name__ == "__main__":
    ...
    for item in test_items:
        ...
        print(
            f"   Breakdown: source={score.source_points}, content={score.content_points}, "
