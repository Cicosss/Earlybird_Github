...
class AlertFeedbackLoop:
    ...
    def process_modification_feedback(
        ...
        try:
            ...
            if reprocessed_analysis:
                ...
                logger.info(
                    f"🔄 [FEEDBACK LOOP] Re-processing completed: {final_result.get('verification_status', 'UNKNOWN')}"
                )
    ...
    def _parse_modifications(
        self, modifications_str: str, alert_data: dict[str, Any]
    ) -> list[AlertModification]:
        """
    ...
    def _parse_market_change(
        self, modifications_str: str, alert_data: dict
    ) -> AlertModification | None:
    ...
    def _reprocess_with_modifications(
        ...
        try:
            ...
            modified_analysis.reasoning = (
                f"Original: {original_summary} | Modifications: {mod_notes}"
            )

            # Mark as reprocessed
            modified_analysis.status = "reprocessed"
            modified_analysis.feedback_loop_iterations = (
                getattr(original_analysis, "feedback_loop_iterations", 0) + 1
            )
    ...
    def _create_modified_context(
        self, original_analysis: NewsLog, modifications: list[AlertModification], context_data: dict
    ) -> dict:
        ...
        try:
            ...
            if not self.verifier:
                ...
                return True, {
                    ...
                    "feedback_loop_iterations": getattr(
                        reprocessed_analysis, "feedback_loop_iterations", 1
                    ),
                }
            ...
            reprocessed_alert_data.update(
                {
                    ...
                    "recommended_market": getattr(
                        reprocessed_analysis,
                        "recommended_market",
                        alert_data.get("recommended_market"),
                    ),
                    "reasoning": getattr(
                        reprocessed_analysis, "reasoning", alert_data.get("reasoning")
                    ),
                    "news_summary": getattr(
                        reprocessed_analysis, "summary", alert_data.get("news_summary")
                    ),
                }
            )
            ...
            reprocessed_context["feedback_loop_iteration"] = getattr(
                reprocessed_analysis, "feedback_loop_iterations", 1
            )
            ...
            if isinstance(verification_result, dict):
                ...
                verification_result["feedback_loop_iterations"] = getattr(
                    reprocessed_analysis, "feedback_loop_iterations", 1
                )
