...
def _tavily_post_match_search(home_team: str, away_team: str, match_date: datetime) -> str | None:
    """
    ...
    try:
        ...
        if response:
            ...
            if insights:
                logger.info(
                    f"🔍 [SETTLER] Tavily post-match insights found for {home_team} vs {away_team}"
                )
                return "\n".join(insights)
...
def get_match_result(home_team: str, away_team: str, match_time: datetime) -> dict | None:
    """
    ...
    try:
        ...
        for match in previous:
            ...
            if (
                home_team.lower() in match_home.lower() or match_home.lower() in home_team.lower()
            ) and (
...
def settle_pending_bets(lookback_hours: int = 48) -> dict:
    ...
    for match_data in matches_to_settle:
        ...
        if match_data["combo_suggestion"]:
            ...
            logger.info(
                f"🧩 [SETTLER] Combo evaluation: {match_data['combo_suggestion']} → {combo_outcome}"
            )
    ...
    with get_db_context() as db:
        try:
            for item in results_cache:
                ...
                if combo_outcome:
                    news_log = (
                        db.query(NewsLog).filter(NewsLog.id == match_data["news_log_id"]).first()
                    )
                    if news_log:
                ...
                # Priority 3: Use current odds (fallback)
                    ...
                    if "home" in market_lower and "win" in market_lower:
                        bet_odds = (
                            match_data["current_home_odd"]
                            if match_data["current_home_odd"]
                            and match_data["current_home_odd"] > 1.0
                            else None
                        )
                    elif "away" in market_lower and "win" in market_lower:
                        bet_odds = (
                            match_data["current_away_odd"]
                            if match_data["current_away_odd"]
                            and match_data["current_away_odd"] > 1.0
                            else None
                        )
                    elif "draw" in market_lower:
                        bet_odds = (
                            match_data["current_draw_odd"]
                            if match_data["current_draw_odd"]
                            and match_data["current_draw_odd"] > 1.0
                            else None
                        )
