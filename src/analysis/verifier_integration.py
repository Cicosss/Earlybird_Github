...
def build_context_data_for_verifier(
    ...
    math_edge: dict | None = None,
    injury_intel: dict | None = None,
    confidence_breakdown: dict | None = None,
    news_source_verification: dict | None = None,
    **kwargs,
) -> dict:
    """
...
def build_news_source_verification(
    news_url: str, news_summary: str, league_key: str | None = None
) -> dict:
    """
    ...
    if source_tier.source_type == "official":
        verification_data["source_guidance"] = (
            "Official source - high reliability but verify for bias"
        )
    elif source_tier.source_type == "broadcaster":
        verification_data["source_guidance"] = (
            "Major broadcaster - generally reliable, check for sensationalism"
        )
    elif source_tier.source_type == "newspaper":
        verification_data["source_guidance"] = (
            "Established newspaper - good track record, verify for clickbait"
        )
    elif source_tier.source_type == "social":
        verification_data["source_guidance"] = (
            "Social media - verify with multiple sources, high rumor risk"
        )
    else:
        verification_data["source_guidance"] = (
            "Unknown source type - requires thorough verification"
        )

    logger.debug(
        f"🔍 [SOURCE VERIFICATION] {domain}: Tier {source_tier.tier}, Weight {source_weight:.2f}, Reliability {reliability}"
    )
