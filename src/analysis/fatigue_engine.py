...
def calculate_fatigue_index(
    team_schedule: list[datetime], match_date: datetime, squad_depth_score: float = 1.0
) -> tuple[float, int]:
    """
...
def get_fatigue_level(fatigue_index: float, hours_since_last: float | None) -> str:
    """
...
def analyze_team_fatigue(
    ...
    if recent_matches and target_match_date:
        fatigue_index, matches_in_window = calculate_fatigue_index(
            recent_matches, target_match_date, squad_depth
        )
    else:
...
def analyze_fatigue_differential(
    home_team: str,
    away_team: str,
    home_hours_since_last: float | None,
    away_hours_since_last: float | None,
    ...
    home_fatigue = analyze_team_fatigue(
        home_team, home_hours_since_last, home_recent_matches, target_match_date
    )

    away_fatigue = analyze_team_fatigue(
        away_team, away_hours_since_last, away_recent_matches, target_match_date
    )
    ...
    if abs(differential) >= 0.3:
        ...
        betting_signal = (
            f"⚡ FATIGUE EDGE: {fresher_team} significativamente più fresco di {tired_team}"
        )
    ...
    if late_game_edge != "NEUTRAL":
        ...
        tired_prob = (
            home_fatigue.late_game_probability
            if late_game_edge == "AWAY"
            else away_fatigue.late_game_probability
        )
        if tired_prob >= 0.40:
            late_signal = (
                f"⏱️ LATE GOAL: {tired_team} a rischio goal dopo 75' ({tired_prob * 100:.0f}%)"
            )
...
def get_enhanced_fatigue_context(
    ...
    if not isinstance(away_fatigue_data, dict):
        away_fatigue_data = {
            "fatigue_level": str(away_fatigue_data) if away_fatigue_data else "Unknown",
            "hours_since_last": None,
        }
