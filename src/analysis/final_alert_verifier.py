...
class FinalAlertVerifier:
    ...
    def verify_final_alert(
        ...
        try:
            ...
            if response:
                ...
                if should_send:
                    logger.info("✅ [FINAL VERIFIER] Alert CONFIRMED for Telegram")
                    return True, result
                else:
                    logger.warning(
                        f"❌ [FINAL VERIFIER] Alert REJECTED: {result.get('rejection_reason', 'Unknown')}"
                    )
    ...
    def _build_verification_prompt(
        ...
        if context_data.get("math_edge"):
            ...
            extracted_lines.append(f"KELLY STAKE: {math_edge.get('kelly_stake', 0):.1f}%")
...
Begin your analysis now."""
        ...
        try:
            processed.update(
                {
                    ...
                    "data_accuracy_score": max(
                        0, min(10, int(response.get("data_accuracy_score", 5)))
                    ),
                    "reasoning_quality_score": max(
                        0, min(10, int(response.get("reasoning_quality_score", 5)))
                    ),
                    ...
                    "adjusted_score_if_discrepancy": max(
                        0, min(10, int(response.get("adjusted_score_if_discrepancy", 5)))
                    ),
                }
            )
            ...
            if source_verification:
                processed["source_verification"] = {
                    ...
                    "cross_source_found": bool(
                        source_verification.get("cross_source_found", False)
                    ),
                    "source_bias_detected": bool(
                        source_verification.get("source_bias_detected", False)
                    ),
                    "source_reliability_adjusted": source_verification.get(
                ...
                logger.info(
                    f"🔍 [SOURCE VERIFICATION] Confirmed: {processed['source_verification']['source_confirmed']}, "
            ...
            if source_verification:
                processed = self._adjust_confidence_based_on_source_verification(
                    processed, source_verification
                )
    ...
    def _adjust_confidence_based_on_source_verification(
        self, processed: dict, source_verification: dict
    ) -> dict:
        """
        ...
        logger.info(
            f"🔍 [CONFIDENCE ADJUSTMENT] Source: {reliability_adjusted}, "
        ...
        if net_impact >= 2:
            ...
            logger.info(
                f"🔍 [CONFIDENCE ADJUSTMENT] Upgraded {current_confidence} → {new_confidence} "
            ...
            logger.warning(
                f"🔍 [CONFIDENCE ADJUSTMENT] Downgraded {current_confidence} → {new_confidence} "
                f"due to source verification issues ({net_impact}): {verification_issues}"
            )

        else:
            logger.debug(
                f"🔍 [CONFIDENCE ADJUSTMENT] Confidence unchanged {current_confidence} "
    ...
    def _handle_discrepancies_intelligently(
        self, processed: dict, discrepancies: list[dict]
    ) -> dict:
        """
        ...
        logger.info(
            f"🔍 [DISCREPANCY HANDLER] Found {total_discrepancies} discrepancies: "
        ...
        if discrepancy_impact == "SEVERE" or high_impact_count >= 2:
            ...
            processed["rejection_reason"] = (
                f"Severe data discrepancies detected: {total_discrepancies} issues, including {high_impact_count} high-impact"
            )
            ...
            goal_discrepancies = [
                d for d in discrepancies if d.get("field") in ["goals", "injuries"]
            ]
            if goal_discrepancies:
                processed["suggested_modifications"] = (
                    "Review goal statistics and injury impact - consider adjusting market or score"
                )
            ...
            processed["data_accuracy_score"] = max(
                3, processed["data_accuracy_score"] - penalty * 2
            )
        ...
        processed["discrepancy_summary"] = {
            ...
            "handling_action": "REJECTED"
            if not processed["should_send"]
            else "CONFIRMED_WITH_ADJUSTMENT",
        }
