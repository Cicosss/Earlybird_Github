...
def get_stats_data() -> dict:
    ...
    try:
        ...
        if os.path.exists(OPTIMIZER_WEIGHTS_FILE):
            with open(OPTIMIZER_WEIGHTS_FILE) as f:
                ...
                for market_type, market_stats in markets.items():
                    ...
                    if market_stats.get("bets", 0) >= 5:
                        ...
                        if league_roi > best_roi:
                            ...
                            best_league_name = (
                                league.replace("soccer_", "").replace("_", " ").title()[:20]
                            )
...
def draw_dashboard(stats: dict = None) -> str:
    ...
    ax.text(
        2.5,
        footer_y,
        f"Best League: {stats['best_league']}",
        fontsize=9,
        color=COLORS["text_secondary"],
        ha="center",
    )
    ax.text(
        7.5,
        footer_y,
        f"Best Driver: {stats['best_driver']}",
        fontsize=9,
        color=COLORS["text_secondary"],
        ha="center",
    )
