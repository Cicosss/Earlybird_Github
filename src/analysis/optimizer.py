...
def categorize_market(market: str) -> str:
    ...
    if "corner" in market_normalized:
        return "CORNERS"
    if (
        "card" in market_normalized
        or "yellow" in market_normalized
        or "booking" in market_normalized
    ):
        return "CARDS"
...
class StrategyOptimizer:
    ...
    def _combine_weights(self, weight1: float, bets1: int, weight2: float, bets2: int) -> float:
        """
...
def get_dynamic_alert_threshold() -> tuple[float, str]:
    ...
    if len(all_returns) < DYNAMIC_THRESHOLD_MIN_BETS:
        return (
            threshold,
            f"Base threshold (aggregate n={len(all_returns)} < {DYNAMIC_THRESHOLD_MIN_BETS})",
        )
    ...
    if adjustments:
        explanation = (
            f"Dynamic: {ALERT_THRESHOLD_BASE} -> {threshold:.1f} ({', '.join(adjustments)})"
        )
    else:
